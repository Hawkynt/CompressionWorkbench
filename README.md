...
| LFD                                                          | `.lfd`        | Yes | Yes         | [LFD resource format](https://web.archive.org/web/20140805170029/http://www.lucasforums.com/showthread.php?t=131803)   | LucasArts X-Wing / TIE Fighter resource bundle — 4-char Type + 8-char Name; auto-emits valid `RMAP` index                                                   |
| OneNote                                                      | `.one`,`.onetoc2` | Yes | -       | [MS-ONESTORE](https://learn.microsoft.com/en-us/openspecs/office_file_formats/ms-onestore/)                            | Microsoft OneNote — pseudo-archive (`FULL.one + metadata.ini` with parse_status=partial); full MS-ONESTORE decoder out of scope                             |
...
| AFS                                                          | `.afs`        | Yes | Yes         | [AFS format wiki](http://wiki.xentax.com/index.php/SEGA_Athena_Filesystem_(AFS))                                       | Sega Athena Filesystem (Dreamcast / PS2 / GameCube) — optional metadata block with timestamps; 0x800 alignment                                              |
| NARC                                                         | `.narc`,`.carc` | Yes | Yes       | [GBATEK NARC](https://problemkaputt.de/gbatek.htm#dscartridgenitrosdkbinaries)                                         | Nintendo DS Archive Resource Compound — flat BTNF tree; BTAF + BTNF + GMIF sections                                                                         |
| SFAR                                                         | `.sfar`       | Yes | -           | [ME3Tweaks SFAR docs](https://me3tweaks.com/me3tweaks-help-and-info/me3-modding-assistant/sfar-files)                  | BioWare Mass Effect 3 DLC — 5-byte LE integers; SHA-1 path hashes; `Filenames.txt` index 0 heuristic for name resolution; LZX blocks not yet decompressed   |
| Avro OCF                                                     | `.avro`       | Yes | -           | [Avro spec](https://avro.apache.org/docs/1.11.1/specification/#object-container-files)                                 | Apache Avro Object Container File — pseudo-archive (`FULL.avro + metadata.ini` with schema, codec, sync-marker); full record decoder out of scope           |
...
| Apache Parquet                                               | `.parquet`    | Yes | -           | [Parquet spec](https://github.com/apache/parquet-format)                                                               | Columnar data — pseudo-archive (`FULL.parquet + metadata.ini` with version/num_rows/schema); Thrift-compact footer walker, full record decode out of scope  |
| Apache Arrow IPC                                             | `.arrow`,`.feather` | Yes | -     | [Arrow IPC spec](https://arrow.apache.org/docs/format/Columnar.html#format-ipc)                                        | Pseudo-archive (`FULL.arrow + metadata.ini` with format/message_count/heuristic_schema); FlatBuffers framing walker, full record decode out of scope        |
| MATLAB MAT v5                                                | `.mat`        | Yes | -           | [MAT v5 spec](https://www.mathworks.com/help/pdf_doc/matlab/matfile_format.pdf)                                        | MATLAB v5 file — pseudo-archive (`FULL.mat + metadata.ini` with description/endian/array_count/arrays); endian-aware (IM/MI), miCOMPRESSED via zlib         |
| PNG                                                          | `.png`        | Yes | -           | [PNG ISO 15948](https://www.iso.org/standard/29581.html)                                                               | Static PNG image — surfaced as colorspace pseudo-archive (`FULL.png + image_000_*/colorspace/...`); IHDR-only metadata read for fast `List`                 |
...
| AKB                                                          | `.akb`        | Yes | Yes         | [vgmstream AKB](https://github.com/vgmstream/vgmstream/blob/master/src/meta/akb.c)                                     | Square Enix audio bank (Final Fantasy / Kingdom Hearts era); raw audio bytes surfaced (no codec decode), `metadata.ini` carries header info                 |
| AWB / AFS2                                                   | `.awb`,`.acb` | Yes | Yes         | [VGMToolbox AFS2](http://wiki.xentax.com/index.php/CRI_Wave_Bank)                                                      | CRI Audio Wave Bank (Capcom / Sega games); endian-agnostic offset width (2/4 bytes); alignment-aware (default 0x20); raw bytes per cue ID                   |
| Web Bundle                                                   | `.wbn`        | Yes | -           | [draft-yasskin-wpack-bundled-exchanges](https://datatracker.ietf.org/doc/draft-yasskin-wpack-bundled-exchanges/)       | Bundled HTTP Exchanges — pseudo-archive (`FULL.wbn + metadata.ini` with version/primary_url/resource_count); minimal CBOR walker (no full decode)           |
| Apache ORC                                                   | `.orc`        | Yes | -           | [ORC format spec](https://orc.apache.org/specification/ORCv1/)                                                         | Optimized Row Columnar — pseudo-archive (`FULL.orc + metadata.ini` with row_count/stripe_count/type_count); Protobuf footer walker, uncompressed only       |
| TFRecord                                                     | `.tfrecord`,`.tfrecords` | Yes | Yes | [TF source](https://github.com/tensorflow/tensorflow/blob/master/tensorflow/core/lib/io/record_writer.h)               | TensorFlow records — length-prefixed payloads with masked CRC-32C (Castagnoli) on length and data; per-record corruption flagged via `IsCorrupt`            |
...
| GAR                                                          | `.gar`        | Yes | Yes         | [3DBrew GAR](https://www.3dbrew.org/wiki/GAR)                                                                          | Nintendo 3DS Generic Asset Resource (Tomodachi Life / Animal Crossing era); type-grouped layout where files sharing an extension share a single type entry  |
| MATLAB MAT v4                                                | `.mat`        | Yes | -           | [MAT v4 spec](https://www.mathworks.com/help/pdf_doc/matlab/matfile_format.pdf)                                        | Older MATLAB v4 file (pre-1996) — pseudo-archive; endian auto-detected from MOPT digit; coexists with MAT v5 (magic-then-extension fallback)                |
...
| VSDX    | `.vsdx`,`.vstx`,`.vssx`,`.vsdm`,`.vstm`,`.vssm` | Yes | Yes | [Visio file formats](https://learn.microsoft.com/en-us/office/client-developer/visio/visio-file-formats)       | Microsoft Visio modern (drawing / template / stencil ± macro)  |
