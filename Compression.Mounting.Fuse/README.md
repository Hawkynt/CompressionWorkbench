...
- positional reads;
- directory snapshots with stable per-handle offsets;
- lookup/forget reference accounting;
...
The native declarations and ABI layouts are derived from the public libfuse3 headers and low-level API documentation (`fuse_lowlevel.h`, `fuse_common.h`) and the upstream `hello_ll` example for lifecycle behavior. The repository does not copy libfuse implementation code.
