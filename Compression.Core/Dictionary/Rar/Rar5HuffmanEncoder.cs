...
internal sealed class Rar5HuffmanEncoder {
  ...
  private static int[] BuildCodeLengths(int[] freq, int numSymbols, int maxBits) {
    ...
    // Build the Huffman tree via a priority queue. Least frequent node first;
    // nodes of equal frequency are taken in the order they were created - the
    // leaves in ascending symbol order, then the internal nodes in merge order.
    // Carrying that node number in the priority makes the ordering total, so
    // the shape of the tree, and with it the code lengths, follow from the
    // frequencies alone and not from how a heap happens to arrange entries it
    // considers equal.
