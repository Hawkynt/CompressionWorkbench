...
/// 64-bit block size, 1–56 byte key, 16-round Feistel network.
/// Initial P-array and S-box values are the hexadecimal digits of pi.
/// </summary>
public sealed class Blowfish {
  ...
  /// <summary>
  /// Initializes a new <see cref="Blowfish"/> instance with the given key.
  /// </summary>
  /// <param name="key">The encryption key, 1 to 56 bytes.</param>
  /// <exception cref="ArgumentException">Thrown when key length is outside 1–56 bytes.</exception>
  public Blowfish(ReadOnlySpan<byte> key) {
    if (key.Length is < 1 or > 56)
      throw new ArgumentException("Key must be 1 to 56 bytes.", nameof(key));
    ...
    // XOR key bytes cyclically into P-array
    var ki = 0;
    for (var i = 0; i < 18; i++) {
      uint data = 0;
      for (var j = 0; j < 4; j++) {
    ...
    // Encrypt zero block iteratively to replace P-array then S-boxes
    uint xl = 0, xr = 0;
  ...
  /// <summary>
  /// Encrypts an 8-byte block in-place (big-endian).
  /// </summary>
  /// <param name="block">The 8-byte block to encrypt.</param>
  /// <exception cref="ArgumentException">Thrown when block is not 8 bytes.</exception>
  public void Encrypt(Span<byte> block) {
    if (block.Length != 8)
      throw new ArgumentException("Block must be 8 bytes.", nameof(block));

    var xl = BinaryPrimitives.ReadUInt32BigEndian(block);
    var xr = BinaryPrimitives.ReadUInt32BigEndian(block[4..]);
    EncryptBlock(ref xl, ref xr);
    BinaryPrimitives.WriteUInt32BigEndian(block, xl);
    BinaryPrimitives.WriteUInt32BigEndian(block[4..], xr);
  }

  /// <summary>
  /// Decrypts an 8-byte block in-place (big-endian).
  /// </summary>
  /// <param name="block">The 8-byte block to decrypt.</param>
  /// <exception cref="ArgumentException">Thrown when block is not 8 bytes.</exception>
  ...
  /// <summary>
  /// Encrypts data using Blowfish-CBC with PKCS7 padding.
  /// </summary>
  /// <param name="data">The plaintext data.</param>
  /// <param name="iv">The 8-byte initialization vector.</param>
  /// <returns>The padded, encrypted data.</returns>
  /// <exception cref="ArgumentException">Thrown when IV is not 8 bytes.</exception>
  public byte[] EncryptCbc(ReadOnlySpan<byte> data, ReadOnlySpan<byte> iv) {
    if (iv.Length != 8)
      throw new ArgumentException("IV must be 8 bytes.", nameof(iv));

    // PKCS7 pad to block boundary
    var padLen = 8 - (data.Length % 8);
    var totalLen = data.Length + padLen;
    var output = new byte[totalLen];
    data.CopyTo(output);
    for (var i = data.Length; i < totalLen; i++)
      output[i] = (byte)padLen;

    // CBC encrypt
    Span<byte> prev = stackalloc byte[8];
    iv.CopyTo(prev);

    for (var i = 0; i < totalLen; i += 8) {
      var block = output.AsSpan(i, 8);
      for (var j = 0; j < 8; j++)
        block[j] ^= prev[j];
      Encrypt(block);
      block.CopyTo(prev);
  ...
  /// <summary>
  /// Decrypts data using Blowfish-CBC with PKCS7 unpadding.
  /// </summary>
  /// <param name="data">The encrypted data; must be a non-zero multiple of 8 bytes.</param>
  /// <param name="iv">The 8-byte initialization vector.</param>
  /// <returns>The decrypted, unpadded plaintext.</returns>
  /// <exception cref="ArgumentException">
  /// Thrown when IV is not 8 bytes, or data length is not a non-zero multiple of 8.
  /// </exception>
  /// <exception cref="InvalidDataException">Thrown when PKCS7 padding is invalid.</exception>
  public byte[] DecryptCbc(ReadOnlySpan<byte> data, ReadOnlySpan<byte> iv) {
    if (iv.Length != 8)
      throw new ArgumentException("IV must be 8 bytes.", nameof(iv));
  ...
  private uint F(uint x) {
    ...
    var c = (x >> 8) & 0xFF;
    var d = x & 0xFF;
    return ((_s0[a] + _s1[b]) ^ _s2[c]) + _s3[d];
  }

  private void EncryptBlock(ref uint xl, ref uint xr) {
    xl ^= _p[0];
    xr ^= F(xl) ^ _p[1];  xl ^= F(xr) ^ _p[2];
    xr ^= F(xl) ^ _p[3];  xl ^= F(xr) ^ _p[4];
    xr ^= F(xl) ^ _p[5];  xl ^= F(xr) ^ _p[6];
    xr ^= F(xl) ^ _p[7];  xl ^= F(xr) ^ _p[8];
    xr ^= F(xl) ^ _p[9];  xl ^= F(xr) ^ _p[10];
    xr ^= F(xl) ^ _p[11]; xl ^= F(xr) ^ _p[12];
    xr ^= F(xl) ^ _p[13]; xl ^= F(xr) ^ _p[14];
    xr ^= F(xl) ^ _p[15]; xl ^= F(xr) ^ _p[16];
    xr ^= _p[17];
    (xl, xr) = (xr, xl);
  }
  ...
  private void DecryptBlock(ref uint xl, ref uint xr) {
    xl ^= _p[17];
    xr ^= F(xl) ^ _p[16]; xl ^= F(xr) ^ _p[15];
    xr ^= F(xl) ^ _p[14]; xl ^= F(xr) ^ _p[13];
    xr ^= F(xl) ^ _p[12]; xl ^= F(xr) ^ _p[11];
    xr ^= F(xl) ^ _p[10]; xl ^= F(xr) ^ _p[9];
    xr ^= F(xl) ^ _p[8];  xl ^= F(xr) ^ _p[7];
    xr ^= F(xl) ^ _p[6];  xl ^= F(xr) ^ _p[5];
    xr ^= F(xl) ^ _p[4];  xl ^= F(xr) ^ _p[3];
    xr ^= F(xl) ^ _p[2];  xl ^= F(xr) ^ _p[1];
    xr ^= _p[0];
    (xl, xr) = (xr, xl);
  }
}
