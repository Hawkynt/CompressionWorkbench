...
| `FileFormat.Ghost`       | WORM  | Reverse-engineered from Symantec Ghost Explorer 2003 + Ghost 1.6 / 2.0 GHOST.EXE (archive.org)               | Modern 3.0 → 11.x with Fast LZ Z1 + zlib Z3-Z9 codecs + CRC-16 stream cipher; pre-3.0 DOS-era Ghost 1.x / 2.x R/O Stage-1 via FE EF head + head-type byte (1=disk / 2=partition / 3=boot); `GhostModifier` provides rebuild-based add/remove as a CLI/UI wrapper helper — the format itself stays WORM (rebuild does not preserve untouched bytes at original offsets) |
