...
| [LZFSE](https://en.wikipedia.org/wiki/LZFSE) | `Lzfse` | `.lzfse` | ✅ | ✅ | — | Entropy-coded `bvx1`/`bvx2` blocks read and `bvx2` written; the writer keeps the smallest of the `bvx2`, LZVN and stored forms per block, so there is nothing left for a separate optimizer pass to choose | [GitHub](https://github.com/lzfse/lzfse) |
...
Every public and protected member of all 1175 types, generated from the built assembly and its XML documentation, is in [REFERENCE.md](https://github.com/Hawkynt/CompressionWorkbench/blob/main/Hawkynt.FileFormats.Archives/REFERENCE.md).
...
- ZPAQ: no ZPAQL virtual machine. StuffIt X and UMX writers emit the envelope shell only. SFAR: LZX payload extraction is limited. Inno Setup: some versions expose no per-file extraction.
