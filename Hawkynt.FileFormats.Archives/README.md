...
| [Inno Setup](https://en.wikipedia.org/wiki/Inno_Setup) | `InnoSetup` |  | WORM | ✅ | — | Extraction plus signature/container output, not a PE installer compiler; some versions expose no per-file extraction | [sourceforge.net](https://sourceforge.net/projects/innounp/) |
...
| --- | --- | --- | :---: | :---: | :---: | --- | --- |
| [ASF / WMV / WMA](https://en.wikipedia.org/wiki/Advanced_Systems_Format) | `Asf` | `.asf` `.wma` `.wmv` | ✅ | ✅ | ✅ | Multi-track codec-preserving mux/remux: Stream Properties, media-object timing, key frames and elementary bytes are carried through unchanged. One Simple Index Object per declared video stream, built from the real key-frame packet positions. Payload-only edits keep opaque Header Object metadata; encrypted streams are refused. | [Microsoft Learn](https://learn.microsoft.com/en-us/windows/win32/wmformat/overview-of-the-asf-format) |
...
- **Media containers.** AVI, MPEG-TS, MPEG-PS, RealMedia and Smacker demux only; MP4, Matroska, ASF, Bink and FLV mux audio tracks only. This is a limit of what demuxing preserves, not of the container specs. The demuxers hand back each track as a codec elementary stream — AVC re-framed as Annex-B, AAC as ADTS, per-PID PES — and per-frame timestamps, interleaving order and the index live in the container layer that is dropped on the way out. Muxing those entries back would mean inventing presentation timing rather than restoring it, so the writers are not there. Closing this needs a demux surface that carries timed packets, not another writer. Bink, Smacker, RealMedia and ASF are additionally reverse-engineered rather than specified. M3U8, PGS `.sup` and VobSub are not container muxes at all: a playlist is a manifest referencing segments it does not contain, and the two subtitle formats are single streams. ISO-BMFF brands are parsed generically, without a brand registry.
...
Every public and protected member of all 1142 types, generated from the built assembly and its XML documentation, is in [REFERENCE.md](https://github.com/Hawkynt/CompressionWorkbench/blob/main/Hawkynt.FileFormats.Archives/REFERENCE.md).
