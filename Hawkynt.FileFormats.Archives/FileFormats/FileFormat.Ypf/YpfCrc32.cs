...
/// where the shared (and hardware-accelerated) <c>Crc32</c> lives. YPF stores this CRC over
/// the on-disk compressed entry bytes (per spec), so a tiny dependency-free table-driven
/// implementation is sufficient.
/// </summary>
public static class YpfCrc32 {
  ...
  private static uint[] BuildTable() {
    ...
    for (var i = 0u; i < 256; ++i) {
      var c = i;
      for (var k = 0; k < 8; ++k)
        c = (c & 1) != 0 ? YpfConstants.Crc32Polynomial ^ (c >> 1) : c >> 1;
      t[i] = c;
    }
    return t;
  }
