...
A **hash function** is the broader concept of deterministically mapping arbitrary input to a fixed-size digest or an extendable output. Non-cryptographic hashes such as xxHash, FNV, CityHash, or MurmurHash optimize speed and distribution. **Cryptographic hashes** additionally aim for preimage, second-preimage, and collision resistance. Those algorithms live in [`Hawkynt.Algorithms.Hashing`](../Hawkynt.Algorithms.Hashing/README.md).
