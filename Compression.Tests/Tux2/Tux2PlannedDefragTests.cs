...
/// <summary>
/// Tux2 lays a container out again by moving whole records, and on one this
...
/// </remarks>
[TestFixture]
public class Tux2PlannedDefragTests {
  ...
  private static MemoryStream Volume(out Dictionary<string, byte[]> files) {
    var work = Path.Combine(Path.GetTempPath(), "cwb_Tux2_" + Guid.NewGuid().ToString("N")[..8]);
    ...
    try {
      ...
      new Tux2FormatDescriptor().Create(image, inputs, new FormatCreateOptions());
      return image;
    } finally {
  ...
  public void Defragment_KeepsEveryPayloadAndTheContainersSize(DefragMode mode) {
    ...
    image.Position = 0;
    new Tux2FormatDescriptor().Defragment(image, new DefragOptions { Mode = mode });
  ...
  public void Defragment_FindsNothingToMoveOnOneOfOurOwn() {
    ...
    image.Position = 0;
    new Tux2FormatDescriptor().Defragment(image,
  ...
  public void Defragment_LeavesTheRecordsWithNothingBetweenThem() {
    using var image = Volume(out _);
    image.Position = 0;
    new Tux2FormatDescriptor().Defragment(image, new DefragOptions { Mode = DefragMode.ConsolidateAtEnd });

    image.Position = 0;
    var records = Tux2RecordMap.Enumerate(image)
