...
/// <summary>
/// Unit tests for <see cref="ExtModifier"/>'s in-flight Add/Remove path —
/// verifies true random-access I/O over an existing ext2/3/4 image: only the
/// superblock, BGD entry, block + inode bitmaps, the new inode slot, the root
/// dir block, and the file's data blocks should be touched. Round-trip +
/// ByteCountingStream perf budget pin against a regression to whole-image I/O.
/// </summary>
[TestFixture]
public class ExtModifierTests {

  [Test, Category("RoundTrip")]
  public void AddFile_ReadsBack() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "hello.txt", "world-ext2"u8.ToArray());

    ms.Position = 0;
    var reader = new ExtReader(ms);
    var entry = reader.Entries.Single(e => e.Name == "hello.txt");
    Assert.That(System.Text.Encoding.UTF8.GetString(reader.Extract(entry)), Is.EqualTo("world-ext2"));
  }

  [Test, Category("RoundTrip")]
  public void AddFile_MultipleFiles_AllReadBack() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "a.txt", "A-data"u8.ToArray());
    ExtModifier.AddFile(ms, "b.txt", "B-data-longer"u8.ToArray());
    ExtModifier.AddFile(ms, "c.txt", "C-data-longest-of-all-three"u8.ToArray());
  ...
  public void AddFile_PreservesPriorEntries() {
    ...
    ExtModifier.AddFile(ms, "added.txt", "ADDED"u8.ToArray());
  ...
  public void AddFile_LargeFileSpanningMultipleBlocks() {
    ...
    ExtModifier.AddFile(ms, "big.bin", data);
  ...
  public void RemoveFile_FreesAndAllowsReuse() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "old.txt", new byte[3 * 1024]);

    Assert.That(ExtModifier.RemoveFile(ms, "old.txt"), Is.True);

    ExtModifier.AddFile(ms, "new.txt", new byte[3 * 1024]);
  ...
  public void RemoveFile_NotFound_ReturnsFalse() {
    using var ms = BuildEmptyImage();
    Assert.That(ExtModifier.RemoveFile(ms, "ghost"), Is.False);
  }

  [Test, Category("RoundTrip")]
  public void RemoveFile_WipesDataBytes() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "secret.txt", "TOPSECRET-MARKER-EXT2"u8.ToArray());
    ExtModifier.RemoveFile(ms, "secret.txt", wipeData: true);

    var asAscii = System.Text.Encoding.ASCII.GetString(ms.ToArray());
    Assert.That(asAscii, Does.Not.Contain("TOPSECRET-MARKER-EXT2"));
  }

  [Test, Category("RoundTrip")]
  public void RemoveFile_NoWipe_LeavesDataBytes() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "secret.txt", "LINGER-MARKER-EXT2"u8.ToArray());
    ExtModifier.RemoveFile(ms, "secret.txt", wipeData: false);

    var asAscii = System.Text.Encoding.ASCII.GetString(ms.ToArray());
    Assert.That(asAscii, Does.Contain("LINGER-MARKER-EXT2"));
  }
  ...
  public void AddSmallFile_TouchesOnlyMetadataAndDataBlocks() {
    ...
    ExtModifier.AddFile(counter, "tiny.txt", "x"u8.ToArray());
  ...
  public void AddTinyFile_DoesNotScaleWithImageSize() {
    ...
    // 4 MiB image; ≤ 16 KiB budget is ~0.4%. Bound at 5% per task spec.
  ...
  public void RemoveSmallFile_DoesNotScaleWithImageSize() {
    using var ms = BuildEmptyImage();
    ExtModifier.AddFile(ms, "tiny.txt", "x"u8.ToArray());
    var counter = new ByteCountingStream(ms);
    Assert.That(ExtModifier.RemoveFile(counter, "tiny.txt"), Is.True);
  ...
  public void Descriptor_RemoveViaInterface() {
    ...
    ((IArchiveModifiable)new ExtFormatDescriptor()).Remove(ms, ["remove-me.txt"]);
  ...
  public void AddFile_UpdatesFreeCounts() {
    ...
    ExtModifier.AddFile(ms, "x.txt", new byte[2 * 1024]); // 2 blocks
  ...
  public void RemoveFile_RestoresFreeCounts() {
    ...
    ExtModifier.AddFile(ms, "x.txt", new byte[2 * 1024]);
    Assert.That(ExtModifier.RemoveFile(ms, "x.txt"), Is.True);
