...
// A companion to WipePreservesLiveDataTests, not a replacement for it. That one
// drives every wipeable format through an edit with a handful of files of a
// kilobyte or two — small enough that NTFS keeps them resident and Btrfs keeps
// them inline, so the map covers them through metadata and the fault below
// cannot appear. This one uses files large enough to own extents, which is the
// case where an incomplete map costs data.
...
/// </remarks>
[TestFixture]
public class WipeAtScaleKeepsFilesTests {
