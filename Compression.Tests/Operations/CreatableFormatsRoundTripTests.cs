...
public sealed class CreatableFormatsRoundTripTests {
  ...
  private static IEnumerable<TestCaseData> BundledCreatableFormats() {
    ...
    var namespaces = DeliveredNamespaces();
    foreach (var d in FormatRegistry.All.OrderBy(x => x.Id, StringComparer.Ordinal)) {
      ...
      if (d.GetType().Assembly.GetName().Name != "Hawkynt.FileFormats.Archives"
  ...
  /// <summary>
  /// The namespaces of the formats the archives package delivers without compiling them — through
  /// its Compression.Core dependency, or inside the bundled copy of Hawkynt.FileFormats.Audio.
  /// Those descriptors report the other assembly's name, so the support-matrix region names them
  /// by namespace instead; everything the package compiles in reports its own assembly.
  /// </summary>
  private static HashSet<string> DeliveredNamespaces() {
    ...
    return Documentation.FilesystemSupportMatrix.DeclaredNamespaces(projectFiles, "Hawkynt.FileFormats.Archives.csproj");
  }
