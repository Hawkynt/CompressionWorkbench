...
public class ConversionMatrixTests {
  ...
  private static readonly Dictionary<string, string> KnownGapTargets =
    new(StringComparer.OrdinalIgnoreCase) {
      ...
      { "IntelHex", "[single-payload/whole-image target] Intel HEX is an address-to-bytes image written as ASCII records; the writer takes one payload and the reader re-lists it as metadata.ini+firmware.bin, not a tree" },
      { "TiTxt", "[single-payload/whole-image target] TI-TXT is an address-to-bytes image written as ASCII lines; the writer takes one payload and the reader re-lists it as metadata.ini+firmware.bin, not a tree" },
      { "UImage", "[single-payload/whole-image target] a legacy uImage is a 64-byte header over one body; the writer takes one payload and the reader re-lists it as metadata.ini+header.bin+payload.bin, not a tree" },
