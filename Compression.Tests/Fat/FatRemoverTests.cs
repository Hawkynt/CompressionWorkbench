...
public class FatRemoverTests {
  ...
  public void RemovedFileContentIsZeroedFromImage() {
    // Marker bytes that are distinctive — easy to search for in the raw image.
    ...
    FatRemover.Remove(image, "TEST.TXT");
  ...
  public void RemovedFileDirEntryIsZeroed() {
    ...
    FatRemover.Remove(image, "GONE.BIN");
    // Search for the filename string — should not remain in the root dir.
    var nameBytes = System.Text.Encoding.ASCII.GetBytes("GONE");
    Assert.That(FindMarker(image, nameBytes), Is.False,
      "filename must be fully wiped from directory, not just marked with 0xE5");
  }

  [Test]
  public void RemovedFileIsNotReadableAfterwards() {
    var image = BuildImageWith(("A.BIN", new byte[] { 1, 2, 3 }), ("B.BIN", new byte[] { 4, 5, 6 }));
    FatRemover.Remove(image, "A.BIN");
  ...
  public void RemovingNonexistentThrows() {
    ...
    Assert.Throws<FileNotFoundException>(() => FatRemover.Remove(image, "MISSING.TXT"));
  }
  ...
  public void DescriptorAsModifiable_AddCombinesFiles() {
    ...
    try {
      ...
      ((IArchiveModifiable)new FatFormatDescriptor()).Add(ms,
