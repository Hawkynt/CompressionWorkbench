...
[TestFixture]
public class CdiInPlaceModifyTests {

  private const int Iso9660SectorSize = 2048;
  private const int RawSectorSize = 2352;
  private const int Mode1DataOffset = 16;
  private const int CdiFooterSize = 8;
  private const uint CdiV3 = 0x80000005;

  // ── geometry fixtures ───────────────────────────────────────────────

  /// <summary>
  /// Builds a raw 2 352-byte/sector Mode 1 CDI image: PVD at LBA 16 with
  /// <c>CD001</c> + 8-byte CDI v3 footer at EOF. Total
  /// <paramref name="sectorCount"/> raw sectors pre-seeded with a deterministic
  /// pattern so we can detect byte-identical preservation.
  /// </summary>
  private static byte[] BuildRawMode1Cdi(int sectorCount = 32) {
    if (sectorCount <= 16) sectorCount = 20;
    var dataLen = sectorCount * RawSectorSize;
    var buf = new byte[dataLen + CdiFooterSize];
    ...
    // CDI v3 footer at EOF.
    System.Buffers.Binary.BinaryPrimitives.WriteUInt32LittleEndian(buf.AsSpan(dataLen, 4), CdiV3);
    System.Buffers.Binary.BinaryPrimitives.WriteUInt32LittleEndian(buf.AsSpan(dataLen + 4, 4), 0u);

    return buf;
  }

  /// <summary>
  /// Builds a cooked 2 048-byte/sector CDI image: PVD at LBA 16, 8-byte CDI
  /// footer at EOF. Locks the geometry-detection path that doesn't require
  /// sync.
  /// </summary>
  private static byte[] BuildCookedCdi(int sectorCount = 32) {
    if (sectorCount <= 16) sectorCount = 20;
  ...
  public void WriteSector_Raw_RewritesUserDataOnly_FramingPreserved() {
    ...
    CdiInPlaceModifier.WriteSector(ms, lba: 20, payload);
    ...
    // CDI footer byte-identical.
    Assert.That(image.AsSpan(image.Length - CdiFooterSize, CdiFooterSize).ToArray(),
      Is.EqualTo(original.AsSpan(original.Length - CdiFooterSize, CdiFooterSize).ToArray()));
  }
  ...
  public void WriteSector_Raw_OtherSectors_ByteIdentical() {
    ...
    var sectorCount = (image.Length - CdiFooterSize) / RawSectorSize;
  ...
  public void WriteSector_Cooked_RewritesUserDataAtSectorOffset() {
    ...
    CdiInPlaceModifier.WriteSector(ms, lba: 22, payload);
  ...
  public void WriteSector_PastEof_AppendsWithCorrectFraming_FooterRelocated() {
    var image = BuildRawMode1Cdi(sectorCount: 20);
    var original = (byte[])image.Clone();
    ...
    CdiInPlaceModifier.WriteSector(ms, lba: 25, payload);

    var grown = ms.ToArray();
    Assert.That(grown.Length, Is.EqualTo(26 * RawSectorSize + CdiFooterSize));

    // Original data-area sectors 0..19 unchanged.
    Assert.That(grown.AsSpan(0, 20 * RawSectorSize).ToArray(),
      Is.EqualTo(original.AsSpan(0, 20 * RawSectorSize).ToArray()));

    // LBA 25 user-data holds payload.
    ...
    // Padding sectors 20..24 carry sync + Mode 1 byte.
    ...
    // CDI footer relocated verbatim to the new EOF.
    Assert.That(grown.AsSpan(grown.Length - CdiFooterSize, CdiFooterSize).ToArray(),
      Is.EqualTo(original.AsSpan(original.Length - CdiFooterSize, CdiFooterSize).ToArray()));
  }
  ...
  public void ZeroSector_WipesUserData_PreservesFramingAndFooter() {
    ...
    Assert.That(CdiInPlaceModifier.ZeroSector(ms, lba: 19), Is.True);
  ...
  public void ZeroSector_PastEof_ReturnsFalse() {
    var image = BuildRawMode1Cdi();
    using var ms = new MemoryStream(image, writable: true);
    Assert.That(CdiInPlaceModifier.ZeroSector(ms, lba: 9999), Is.False);
  }
  ...
  public void Descriptor_Add_RewritesNamedSector() {
    ...
    ((IArchiveModifiable)new CdiFormatDescriptor()).Add(ms, [
      ArchiveInputInfo.InMemory(CdiInPlaceModifier.FormatSectorEntryName(20), payload),
    ]);
  ...
  public void Descriptor_Add_AppendsPastEofPreservingDataAndFooter() {
    ...
    ((IArchiveModifiable)new CdiFormatDescriptor()).Add(ms, [
      ArchiveInputInfo.InMemory(CdiInPlaceModifier.FormatSectorEntryName(30), payload),
    ]);

    var grown = ms.ToArray();
    // Original data sectors preserved.
    Assert.That(grown.AsSpan(0, 20 * RawSectorSize).ToArray(),
      Is.EqualTo(original.AsSpan(0, 20 * RawSectorSize).ToArray()));
  ...
  public void Descriptor_Remove_ZerosUserData_LeavesEverythingElseIdentical() {
    ...
    ((IArchiveModifiable)new CdiFormatDescriptor()).Remove(ms, [
      CdiInPlaceModifier.FormatSectorEntryName(20),
    ]);
  ...
  public void Descriptor_Replace_RewritesUserDataPreservingFramingAndNeighbours() {
    ...
    var modifier = (IArchiveModifiable)new CdiFormatDescriptor();
    modifier.Add(ms, [ArchiveInputInfo.InMemory(CdiInPlaceModifier.FormatSectorEntryName(21), v1)]);
    modifier.Add(ms, [ArchiveInputInfo.InMemory(CdiInPlaceModifier.FormatSectorEntryName(21), v2)]);
  ...
  public void WriteSector_WrongUserDataLength_Throws() {
    using var ms = new MemoryStream(BuildRawMode1Cdi(), writable: true);
    Assert.Throws<ArgumentException>(() =>
      CdiInPlaceModifier.WriteSector(ms, 20, new byte[100]));
  }
  ...
  public void WriteSector_NegativeLba_Throws() {
    ...
    Assert.Throws<ArgumentOutOfRangeException>(() =>
      CdiInPlaceModifier.WriteSector(ms, -1, new byte[Iso9660SectorSize]));
  }

  [Test, Category("HappyPath")]
  public void Descriptor_AdvertisesCanModify() {
    var d = new CdiFormatDescriptor();
    Assert.That(d.Capabilities.HasFlag(FormatCapabilities.CanModify), Is.True);
  }

  [Test, Category("HappyPath")]
  public void Descriptor_ImplementsIArchiveModifiable() {
    Assert.That(new CdiFormatDescriptor(), Is.InstanceOf<IArchiveModifiable>());
  }

  [Test, Category("HappyPath")]
  public void TryParseSectorEntryName_RoundTrips() {
    var name = CdiInPlaceModifier.FormatSectorEntryName(12345);
    Assert.That(CdiInPlaceModifier.TryParseSectorEntryName(name, out var lba), Is.True);
    Assert.That(lba, Is.EqualTo(12345));
  }

  [Test, Category("Boundary")]
  public void TryParseSectorEntryName_BogusNames_Rejected() {
    Assert.That(CdiInPlaceModifier.TryParseSectorEntryName("readme.txt", out _), Is.False);
    Assert.That(CdiInPlaceModifier.TryParseSectorEntryName("sector-abc.bin", out _), Is.False);
    Assert.That(CdiInPlaceModifier.TryParseSectorEntryName("sector-.bin", out _), Is.False);
    Assert.That(CdiInPlaceModifier.TryParseSectorEntryName("", out _), Is.False);
  }
  ...
  [Test, Category("Boundary")]
  public void DetectGeometry_StripsCdiFooter_FromDataAreaLength() {
    var image = BuildRawMode1Cdi(sectorCount: 32);
    using var ms = new MemoryStream(image, writable: true);
    var geom = CdiInPlaceModifier.DetectGeometry(ms);
