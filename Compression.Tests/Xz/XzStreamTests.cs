...
public class XzStreamTests {
  ...
  public void RoundTrip_TextData() {
    var data = "Hello, XZ World! Testing LZMA2-based compression."u8.ToArray();
    var compressed = CompressWithOurs(data);
    var result = DecompressWithOurs(compressed);
    Assert.That(result, Is.EqualTo(data));
  }
  ...
  public void CheckType_Crc32_Works() {
    var data = "CRC-32 check test"u8.ToArray();
  ...
  public void CheckType_Crc64_Works() {
    var data = "CRC-64 check test"u8.ToArray();
    var compressed = CompressWithOurs(data); // Default is CRC-64
    var result = DecompressWithOurs(compressed);
    Assert.That(result, Is.EqualTo(data));
  }
  ...
  public void RoundTrip_Sha256Check() {
    var data = "XZ with SHA-256 check type test data."u8.ToArray();
    using var ms = new MemoryStream();
    using (var xz = new XzStream(ms, CompressionStreamMode.Compress,
  ...
  public void RoundTrip_Crc32Check() {
    var data = "XZ with CRC-32 check type test data."u8.ToArray();
    using var ms = new MemoryStream();
    using (var xz = new XzStream(ms, CompressionStreamMode.Compress,
      1 << 20, XzConstants.CheckCrc32, leaveOpen: true))
      xz.Write(data, 0, data.Length);
