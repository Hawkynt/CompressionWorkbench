...
public sealed class FilesystemMountCapabilityResolverTests {
  ...
  public void DescriptorCanModifyAloneDoesNotGrantWritableMount() {
    ...
    var plan = Resolve(Profile(canMountWritable: false), MountAccessMode.ReadWrite, sourceCanWrite: true);
    Assert.Multiple(() => {
      Assert.That(plan.IsSupported, Is.False);
      Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Contain(MountSupportReasonCode.FilesystemProfileNotWritable));
      Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Not.Contain(MountSupportReasonCode.MissingDriverCapabilities));
    });
  }
  ...
  public void ReadOnlySourceRejectsWritableMount() {
    ...
    Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Contain(MountSupportReasonCode.SourceIsReadOnly));
  }
  ...
  public void MissingCoreWritePrimitiveRejectsWritableMount(FilesystemDriverCapabilities missingCapability) {
    var plan = Resolve(Profile(capabilities: AllCoreCapabilities & ~missingCapability), MountAccessMode.ReadWrite, sourceCanWrite: true);
    var missingReason = plan.Reasons.Single(reason =>
      reason.Code == MountSupportReasonCode.MissingDriverCapabilities && reason.MissingCapabilities.HasFlag(missingCapability));
    Assert.Multiple(() => {
      Assert.That(plan.IsSupported, Is.False);
  ...
  public void MissingCoreReadPrimitiveRejectsReadOnlyMount(FilesystemDriverCapabilities missingCapability) {
    var plan = Resolve(Profile(capabilities: AllCoreCapabilities & ~missingCapability), MountAccessMode.ReadOnly, sourceCanWrite: false);
  ...
  public void UnavailableBackendRejectsEveryAccessMode(MountAccessMode accessMode) {
    var plan = Resolve(Profile(), accessMode, sourceCanWrite: true, backend: Backend(isAvailable: false));
    Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Contain(MountSupportReasonCode.BackendUnavailable));
  }

  [Test]
  public void BackendReadOnlySupportFlagIsRequired() {
    var plan = Resolve(Profile(), MountAccessMode.ReadOnly, sourceCanWrite: false, backend: Backend(supportsReadOnly: false));
    Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Contain(MountSupportReasonCode.BackendDoesNotSupportReadOnly));
  }

  [Test]
  public void BackendReadWriteSupportFlagIsRequired() {
    var plan = Resolve(Profile(), MountAccessMode.ReadWrite, sourceCanWrite: true, backend: Backend(supportsReadWrite: false));
    Assert.That(plan.Reasons.Select(static reason => reason.Code), Does.Contain(MountSupportReasonCode.BackendDoesNotSupportReadWrite));
  }

  [Test]
  public void FlatFilesystemCanQualifyForWritableMount() {
    var optionalDirectoryMutations = FilesystemDriverCapabilities.CreateDirectory | FilesystemDriverCapabilities.RemoveDirectory;
    var profile = Profile(capabilities: AllCoreCapabilities & ~optionalDirectoryMutations);
    var plan = Resolve(profile, MountAccessMode.ReadWrite, sourceCanWrite: true);
    Assert.Multiple(() => {
      Assert.That(plan.IsSupported, Is.True);
      Assert.That(plan.RequiredCapabilities & optionalDirectoryMutations, Is.EqualTo(FilesystemDriverCapabilities.None));
      Assert.That(plan.MissingCapabilities, Is.EqualTo(FilesystemDriverCapabilities.None));
    });
  }
  ...
  public void BackendSpecificCapabilitiesAreReportedExactly() {
    ...
    Assert.Multiple(() => {
      ...
      Assert.That(plan.Reasons.Where(static reason => reason.Code == MountSupportReasonCode.MissingDriverCapabilities)
        .Select(static reason => reason.MissingCapabilities), Is.EquivalentTo(new[] { requiredRead, requiredWrite }));
    });
  }
  ...
  public void RegistryRejectsDuplicateBackendIdsCaseInsensitively() {
    ...
    Assert.That(() => new MountBackendRegistry(new IFilesystemMountBackend[] { first, duplicate }), Throws.ArgumentException);
  }

  private static MountPlan Resolve(FilesystemDriverProfile profile, MountAccessMode accessMode, bool sourceCanWrite, MountBackendProfile? backend = null)
    => FilesystemMountCapabilityResolver.Resolve(profile, backend ?? Backend(), accessMode, sourceCanWrite);
  ...
  ) => new("testfs", "synthetic", capabilities ?? AllCoreCapabilities, mutationModel, canMount, canMountWritable, Array.Empty<string>());
  ...
  ) => new(id, "Synthetic backend", isAvailable, supportsReadOnly, supportsReadWrite, requiredReadCapabilities, requiredWriteCapabilities, Array.Empty<string>());
  ...
  private sealed class StubBackend(MountBackendProfile profile) : IFilesystemMountBackend {
    ...
    public ValueTask<IMountSession> MountAsync(FilesystemMountRequest request, CancellationToken cancellationToken = default)
