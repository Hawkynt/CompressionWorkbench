...
/// Format creation support is intentionally not used as a mount qualification:
/// writable formats must opt in through a real filesystem driver profile and a
/// format-specific persistence contract.
/// </summary>
[TestFixture]
public sealed class FilesystemFormatMountIntegrationTests {
  ...
  public async Task RealFormatsSurviveRepeatedMountReadOnlyMutationAttemptsAndUnmounts(
    ...
    for (var cycle = 0; cycle < 3; ++cycle) {
      ...
      Assert.That(fixture.ProbedProfile.CanMountWritable, Is.EqualTo(expectWritableFilesystemProfile),
  ...
  [Test]
  [Category("Mounting")]
  public void WritableFilesystemProfilesAreNotSilentlyDowngradedByBackends() {
    using var fixture = CreateFixture("D64");
    Assert.That(fixture.ProbedProfile.CanMountWritable, Is.True,
      "D64 is the regression fixture for an independently implemented mounted-write filesystem provider.");

    foreach (var backend in BackendProfiles()) {
      var plan = FilesystemMountCapabilityResolver.Resolve(
        fixture.ProbedProfile,
        backend,
        MountAccessMode.ReadWrite,
        sourceCanWrite: true);

      if (backend.SupportsReadWrite && backend.IsAvailable) {
      ...
      Assert.That(plan.Reasons.Any(static reason =>
          reason.Code is MountSupportReasonCode.BackendDoesNotSupportReadWrite or MountSupportReasonCode.BackendUnavailable),
        Is.True,
        $"{backend.Id}: writable refusal must identify backend support/runtime availability rather than fail later inside a native callback.");
    }
  }
  ...
  private static FormatFixture CreateFixture(string formatId) {
    ...
    return new(formatId, image, filesystem, profile, FindReadableFile(filesystem));
  }
  ...
  private sealed class FormatFixture(
    ...
    public FilesystemDriverProfile ProbedProfile { get; } = probedProfile;
