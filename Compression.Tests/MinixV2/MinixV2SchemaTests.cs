...
/// <summary>
/// Verifies the Minix v2 creation-option schema: the published NameLength knob
/// is real (it flips the superblock magic between the 14-byte 0x2468 and the
/// 30-byte 0x2478 variant) and files still round-trip through the reader.
/// </summary>
[TestFixture]
public class MinixV2SchemaTests {
  ...
  public void Descriptor_ExposesNameLengthSchema() {
    var desc = (IFormatOptionsSchema)new MinixV2FormatDescriptor();
  ...
  public void Create_WithNameLength30_SetsLongNameMagicAndRoundTrips() {
    var desc = new MinixV2FormatDescriptor();
    var opts = new FormatCreateOptions {
      FormatSpecific = new Dictionary<string, string> { ["NameLength"] = "30" },
    };

    using var ms = new MemoryStream();
    desc.Create(ms, [ArchiveInputInfo.InMemory("a-longer-file-name.txt", "minix-v2"u8.ToArray())], opts);
    var image = ms.ToArray();

    var magic = BinaryPrimitives.ReadUInt16LittleEndian(image.AsSpan(MagicOffset, 2));
    Assert.That(magic, Is.EqualTo(0x2478), "NameLength=30 must select the 0x2478 magic.");
  ...
  public void Create_DefaultNameLength14_UsesShortNameMagic() {
    ...
    var image = ms.ToArray();
    var magic = BinaryPrimitives.ReadUInt16LittleEndian(image.AsSpan(MagicOffset, 2));
    Assert.That(magic, Is.EqualTo(0x2468));
  }
}
