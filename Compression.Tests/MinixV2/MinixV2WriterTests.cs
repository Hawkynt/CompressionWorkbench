...
[TestFixture]
public class MinixV2WriterTests {

  private static readonly (string Path, string Content)[] SampleTree = [
    ("readme.txt", "Top-level readme for the Minix v2 volume."),
  ...
  public void Build_NestedFiles_RoundTripsThroughReader() {
    ...
    var reader = new MinixV2Reader(ms);

    Assert.That(reader.Magic, Is.EqualTo((ushort)0x2468));
  ...
  public void Descriptor_IsCreatable_AndCreateProducesReadableImage() {
    var d = new MinixV2FormatDescriptor();
  ...
  public void Build_Image_PassesFsckMinix() {
    ...
    var image = BuildSampleImage();
    var tmp = Path.Combine(Path.GetTempPath(), $"minixv2-{Guid.NewGuid():N}.img");
