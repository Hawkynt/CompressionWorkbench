...
using System.Text;
using Codec.Pcm;
using FileFormat.Bcstm;

namespace Compression.Tests.Bcstm;
...
public class BcstmTests {
  ...
  public void Writer_Reader_RoundTripsStructureAndCloseness() {
    ...
    var blob = new BcstmWriter().Write([left, right], 32000);

    Assert.That(blob.AsSpan(0, 4).ToArray(), Is.EqualTo("CSTM"u8.ToArray()));
    ...
    Assert.That(parsed.Info.Codec, Is.EqualTo(2));
  ...
  public void Descriptor_ListsFullAndPerChannelAndMetadata() {
    var blob = new BcstmWriter().Write([MakeTone(5000, 40, 10000), MakeTone(5000, 60, 8000)], 22050);
    using var ms = new MemoryStream(blob);
    var entries = new BcstmFormatDescriptor().List(ms, null);

    Assert.That(entries.Any(e => e.Name == "FULL.bcstm" && e.Kind == "Container"), Is.True);
  ...
  public void Descriptor_Metadata_DescribesStream() {
    var blob = new BcstmWriter().Write([MakeTone(1000, 25, 8000), MakeTone(1000, 25, 8000)], 48000);
    using var ms = new MemoryStream(blob);
    using var output = new MemoryStream();
    new BcstmFormatDescriptor().ExtractEntry(ms, "metadata.ini", output, null);
  ...
  public void Descriptor_FullOnlyFallback_OnGarbage() {
    var blob = "CSTM"u8.ToArray().Concat(new byte[0x40]).ToArray();
    BinaryPrimitives.WriteUInt16LittleEndian(blob.AsSpan(4), 0xFEFF);
    using var ms = new MemoryStream(blob);
    var entries = new BcstmFormatDescriptor().List(ms, null);
    Assert.That(entries.Count, Is.EqualTo(1));
    Assert.That(entries[0].Name, Is.EqualTo("FULL.bcstm"));
  }

  [Test]
  public void Create_FromPerChannelWavs_ProducesReadableBcstm() {
    ...
    using var output = new MemoryStream();
    new BcstmFormatDescriptor().Create(output, inputs, new Compression.Registry.FormatCreateOptions());
    var parsed = new BcstmReader().Read(output.ToArray());
    Assert.That(parsed.Info.NumChannels, Is.EqualTo(2));
  ...
  public void Create_FullPassthrough_ReturnsVerbatim() {
    var original = new BcstmWriter().Write([MakeTone(2000, 30, 8000)], 16000);
    var inputs = new List<Compression.Registry.ArchiveInputInfo> {
      Compression.Registry.ArchiveInputInfo.InMemory("FULL.bcstm", original),
    };
    using var output = new MemoryStream();
    new BcstmFormatDescriptor().Create(output, inputs, new Compression.Registry.FormatCreateOptions());
  ...
  public void Writer_MultiBlock_RoundTrips() {
    ...
    var blob = new BcstmWriter().Write([mono], 32000);
    var parsed = new BcstmReader().Read(blob);
