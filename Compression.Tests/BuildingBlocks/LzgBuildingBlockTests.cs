using System.Text;
using Compression.Core.Dictionary.Lzg;

namespace Compression.Tests.BuildingBlocks;

[TestFixture]
public class LzgBuildingBlockTests {

  private static readonly LzgBuildingBlock Bb = new();
  ...
  public void EscapeByteLiteral_RoundTrips() {
    byte[] data = [0xFF, 0xFF, 0x01, 0xFF, 0x02];
    var round = Bb.Decompress(Bb.Compress(data));
    Assert.That(round, Is.EqualTo(data).AsCollection);
  }
  ...
  public void EnglishText_RoundTrips() {
    var data = Encoding.ASCII.GetBytes(
      "LZG is Marcus Geelnard's small dependency-free LZ77 codec. LZG is Marcus Geelnard's small dependency-free LZ77 codec.");
    var compressed = Bb.Compress(data);
    var round = Bb.Decompress(compressed);
    Assert.That(round, Is.EqualTo(data).AsCollection);
  ...
  public void Registry_Metadata_IsStable() {
    Assert.Multiple(() => {
      Assert.That(Bb.Id, Is.EqualTo("BB_Lzg"));
      Assert.That(Bb.DisplayName, Is.EqualTo("LZG"));
