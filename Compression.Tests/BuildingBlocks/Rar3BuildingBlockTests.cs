...
[TestFixture]
public class Rar3BuildingBlockTests {

  private static readonly Rar3BuildingBlock Bb = new();
  ...
  public void Registry_Metadata_IsDistinctFromRar5() {
    var rar5 = new RarBuildingBlock();
    Assert.Multiple(() => {
      Assert.That(Bb.Id, Is.EqualTo("BB_Rar3"));
      Assert.That(Bb.DisplayName, Is.EqualTo("RAR3 (classic)"));
      Assert.That(Bb.Family, Is.EqualTo(Compression.Registry.AlgorithmFamily.Dictionary));
      Assert.That(Bb.Id, Is.Not.EqualTo(rar5.Id));
      Assert.That(Bb.DisplayName, Is.Not.EqualTo(rar5.DisplayName));
      Assert.That(Bb.Description, Is.Not.EqualTo(rar5.Description));
    });
  }
}
