using System.Text;
using Compression.Core.Dictionary.Lzturbo;
using Compression.Registry;

namespace Compression.Tests.BuildingBlocks;

[TestFixture]
public class LzturboBuildingBlockTests {

  private static readonly LzturboBuildingBlock Bb = new();
  ...
  public void RejectsBadMagic() {
    var compressed = Bb.Compress("hello"u8.ToArray());
    compressed[0] = (byte)~compressed[0];
    Assert.Throws<InvalidDataException>(() => Bb.Decompress(compressed));
  }

  [Test, Category("EdgeCase")]
  public void BbRegistry_ExposesMetadata() {
    Assert.That(Bb.Id, Is.EqualTo("BB_Lzturbo"));
    Assert.That(Bb.DisplayName, Is.EqualTo("LZTURBO"));
