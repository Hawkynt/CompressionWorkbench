using System.Text;
using Compression.Core.Dictionary.Lzrw1;
using Compression.Registry;

namespace Compression.Tests.BuildingBlocks;

[TestFixture]
public class Lzrw1Tests {

  private static readonly Lzrw1BuildingBlock Bb = new();
  ...
  public void TextSample_RoundTrips() {
    var data = Encoding.ASCII.GetBytes(string.Concat(Enumerable.Repeat(
      "Jackdaws love my big sphinx of quartz. ", 20)));
    var compressed = Bb.Compress(data);
    var round = Bb.Decompress(compressed);
    Assert.That(round, Is.EqualTo(data).AsCollection);
  ...
  public void BbRegistry_ExposesMetadata() {
    Assert.That(Bb.Id, Is.EqualTo("BB_Lzrw1"));
    Assert.That(Bb.DisplayName, Is.EqualTo("LZRW1"));
