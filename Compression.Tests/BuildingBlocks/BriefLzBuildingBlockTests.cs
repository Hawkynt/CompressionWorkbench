using System.Text;
using Compression.Core.Dictionary.BriefLz;

namespace Compression.Tests.BuildingBlocks;

[TestFixture]
public class BriefLzBuildingBlockTests {

  private static readonly BriefLzBuildingBlock Bb = new();
  ...
  public void EnglishText_RoundTrips() {
    var data = Encoding.ASCII.GetBytes(
      "BriefLZ is a fast, small, and simple LZ77-based compression library. " +
      "BriefLZ is a fast, small, and simple LZ77-based compression library.");
    var compressed = Bb.Compress(data);
    var round = Bb.Decompress(compressed);
    Assert.That(round, Is.EqualTo(data).AsCollection);
  ...
  public void Registry_Metadata_IsStable() {
    Assert.Multiple(() => {
      Assert.That(Bb.Id, Is.EqualTo("BB_BriefLz"));
      Assert.That(Bb.DisplayName, Is.EqualTo("BriefLZ"));
