...
/// <summary>
/// ReiserFS is now read+write (<see cref="IArchiveModifiable"/>) via the
/// read-extract-rebuild path: add / replace / remove rebuild a fresh
/// reiserfsck-clean image with the writer. These tests pin that the mutations
/// round-trip through <see cref="ReiserFsReader"/>.
/// </summary>
[TestFixture]
public class ReiserFsModifyTests {
  ...
  private static Dictionary<string, byte[]> ReadAll(MemoryStream image) {
    image.Position = 0;
    var r = new ReiserFsReader(image);
