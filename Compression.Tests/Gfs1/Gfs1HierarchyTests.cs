...
[TestFixture]
public class Gfs1HierarchyTests {
  ...
  public void List_ReturnsFiveFiles_AtCorrectNestedPaths() {
    ...
    var entries = new Gfs1FormatDescriptor().List(ms, null);
  ...
  public void Extract_ReproducesTreeOnDisk() {
    var img = BuildSyntheticImage();
    using var ms = new MemoryStream(img);
    var tmp = Path.Combine(Path.GetTempPath(), $"gfs1-hier-{Guid.NewGuid():N}");
    Directory.CreateDirectory(tmp);
    try {
      new Gfs1FormatDescriptor().Extract(ms, tmp, null, null);
