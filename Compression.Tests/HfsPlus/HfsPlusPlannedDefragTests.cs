...
public class HfsPlusPlannedDefragTests {
  ...
  private static string Scratch(byte[] data) {
    var path = Path.Combine(_Scratch.Value, "cwb_hfsplus_" + Guid.NewGuid().ToString("N")[..8]);
  ...
  private static readonly Lazy<string> _Scratch = new(() => {
    var dir = Path.Combine(Path.GetTempPath(), "cwb_hfsplus_scratch_" + Guid.NewGuid().ToString("N")[..8]);
