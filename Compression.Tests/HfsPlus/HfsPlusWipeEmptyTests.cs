...
/// <summary>
/// Cluster-tip / unused-space wiping for the HFS+ descriptor. HFS+ uses a
/// 4096-byte default allocation block; the slack between a file's logical size
/// and the end of its last block is the block tip and must be zeroed. The
/// catalog extent map clamps each file's run to its logical length, so the tip
/// is a free gap the generic wiper zero-fills.
/// </summary>
[TestFixture]
public class HfsPlusWipeEmptyTests {
  ...
  [Test]
  public void HfsPlusDescriptorImplementsIWipeEmpty() {
  ...
  public void WipeEmpty_ClusterTip_ZeroedAndFileRoundTrips() {
    var fileSize = BlockSize - 500; // 3596 bytes in a 4096-byte block — leaves a tip.
    ...
    ms.Position = 0;
    using var reader = new HfsPlusReader(ms);
    var entry = reader.Entries.First(e => !e.IsDirectory && e.FullPath == "secret.bin");
