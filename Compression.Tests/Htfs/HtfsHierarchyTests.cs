...
[TestFixture]
public class HtfsHierarchyTests {
  ...
  public void List_ReturnsFiveFiles_AtCorrectNestedPaths() {
    ...
    var entries = new HtfsFormatDescriptor().List(ms, null);
  ...
  public void Extract_ReproducesTreeOnDisk() {
    var img = BuildSyntheticImage();
    using var ms = new MemoryStream(img);
    var tmp = Path.Combine(Path.GetTempPath(), $"htfs-hier-{Guid.NewGuid():N}");
    Directory.CreateDirectory(tmp);
    try {
      new HtfsFormatDescriptor().Extract(ms, tmp, null, null);
  ...
  public void Descriptor_AdvertisesNewCapabilities() {
    var d = new HtfsFormatDescriptor();
    Assert.Multiple(() => {
