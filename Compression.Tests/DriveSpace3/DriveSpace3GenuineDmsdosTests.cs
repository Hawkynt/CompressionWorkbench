...
public class DriveSpace3GenuineDmsdosTests {
  ...
  public void GenuineDvr3_IsMountedAndReadByteExact_ByRealDmsdosDriver() {
    ...
    try {
      ...
      foreach (var (name, expected) in files) {
        ...
        var payload = StripScanDirPrefix(raw);
  ...
  // dcread prints a single "scan_dir: searching for NAME in N\n" diagnostic line
  // to stdout before the raw cluster bytes, and emits the whole (untruncated)
  // cluster. Drop that one leading line if present; the caller compares the
  // first <filesize> bytes.
  private static byte[] StripScanDirPrefix(byte[] output) {
    if (output.Length >= 9 &&
        Encoding.ASCII.GetString(output, 0, 9) == "scan_dir:") {
      var nl = Array.IndexOf(output, (byte)'\n');
      if (nl >= 0 && nl + 1 <= output.Length)
        return output[(nl + 1)..];
    }
    return output;
  }
