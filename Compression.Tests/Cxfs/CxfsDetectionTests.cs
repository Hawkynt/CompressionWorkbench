...
public class CxfsDetectionTests {
  ...
  [Test, Category("HappyPath")]
  public void Create_AuthorsNestedDirectoriesAsShortFormXfsDirectories() {
    var d = new CxfsFormatDescriptor();
    using var image = new MemoryStream();
    var nested = "nested"u8.ToArray();
    var deep = "deep"u8.ToArray();

    d.Create(image, [
      ArchiveInputInfo.InMemory("root.bin", "root"u8.ToArray()),
      new ArchiveInputInfo("", "empty/", true),
      ArchiveInputInfo.InMemory("dir/nested.bin", nested),
      ArchiveInputInfo.InMemory("dir/sub/deep.bin", deep),
    ], new FormatCreateOptions());

    image.Position = 0;
    var entries = ((IArchiveFormatOperations)d).List(image, null);
    var names = entries.Select(e => e.Name).ToArray();

    Assert.Multiple(() => {
      Assert.That(names, Does.Contain("root.bin"));
      Assert.That(names, Does.Contain("empty"));
      Assert.That(names, Does.Contain("dir"));
      Assert.That(names, Does.Contain("dir/nested.bin"));
      Assert.That(names, Does.Contain("dir/sub"));
      Assert.That(names, Does.Contain("dir/sub/deep.bin"));
      Assert.That(entries.Single(e => e.Name == "dir").IsDirectory, Is.True);
      Assert.That(entries.Single(e => e.Name == "dir/sub").IsDirectory, Is.True);
      Assert.That(Extract(d, image, "dir/nested.bin"), Is.EqualTo(nested));
      Assert.That(Extract(d, image, "dir/sub/deep.bin"), Is.EqualTo(deep));
    });
  }

  [Test, Category("ErrorHandling")]
  public void Create_RejectsTraversalAndEmptyPathComponents() {
    var d = new CxfsFormatDescriptor();
    using var image = new MemoryStream();

    Assert.Throws<NotSupportedException>(() =>
      d.Create(image, [ArchiveInputInfo.InMemory("dir/../escape.txt", "x"u8.ToArray())], new FormatCreateOptions()));
    Assert.Throws<NotSupportedException>(() =>
      d.Create(image, [ArchiveInputInfo.InMemory("dir//file.txt", "x"u8.ToArray())], new FormatCreateOptions()));
  }

  [Test, Category("HappyPath")]
  public void Add_PlacesAnEntryInsideAnExistingSubdirectory() {
    var d = new CxfsFormatDescriptor();
    using var image = new MemoryStream();
    d.Create(image, [ArchiveInputInfo.InMemory("dir/first.bin", "first"u8.ToArray())], new FormatCreateOptions());

    var added = "second"u8.ToArray();
    image.Position = 0;
    d.Add(image, [ArchiveInputInfo.InMemory("dir/second.bin", added)]);

    image.Position = 0;
    var names = ((IArchiveFormatOperations)d).List(image, null).Select(e => e.Name).ToArray();
    Assert.Multiple(() => {
      Assert.That(names, Does.Contain("dir/first.bin"));
      Assert.That(names, Does.Contain("dir/second.bin"));
      Assert.That(Extract(d, image, "dir/second.bin"), Is.EqualTo(added));
    });
  }
