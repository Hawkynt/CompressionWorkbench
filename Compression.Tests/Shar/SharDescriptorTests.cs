...
[TestFixture]
public class SharDescriptorTests {

  [Test, Category("HappyPath")]
  public void Descriptor_Properties() {
    var desc = new FileFormat.Shar.SharFormatDescriptor();

    Assert.That(desc.Id, Is.EqualTo("Shar"));
    Assert.That(desc.DefaultExtension, Is.EqualTo(".shar"));
    Assert.That(desc.Category, Is.EqualTo(Compression.Registry.FormatCategory.Archive));
    Assert.That(desc.Extensions, Contains.Item(".sh"));
  }
  ...
  public void RoundTrip_ViaInterface() {
    ...
    try {
      var data = "Hello SHAR archive!"u8.ToArray();
