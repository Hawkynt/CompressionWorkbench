...
[TestFixture]
public class CompoundTarLzipTests {

  [Test, Category("RoundTrip"), Category("End2End")]
  public void TarLzip_RoundTrip() {
    var data = "TarLzip compound format test content for round-trip verification."u8.ToArray();
    var dir = Path.Combine(Path.GetTempPath(), "cwb_test_tarlzip_" + Guid.NewGuid().ToString("N")[..8]);
    var archive = Path.Combine(dir, "test.tar.lz");
  ...
  public void TarLzip_DetectByExtension() {
    var format = Compression.Lib.FormatDetector.DetectByExtension("test.tar.lz");
    Assert.That(format, Is.EqualTo(Compression.Lib.FormatDetector.Format.TarLzip));
  }
}
