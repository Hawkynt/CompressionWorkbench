...
using System.Text;
using Codec.Pcm;
using FileFormat.Hps;

namespace Compression.Tests.Hps;
...
public class HpsTests {
  ...
  public void Writer_Reader_RoundTripsStructureAndCloseness() {
    ...
    var blob = new HpsWriter().Write([left, right], 32000);

    Assert.That(blob.AsSpan(0, 8).ToArray(), Is.EqualTo(" HALPST\0"u8.ToArray()));
  ...
  [Test]
  public void Writer_Mono_SampleCountRecoveredExactly() {
    foreach (var n in new[] { 13, 14, 15, 28, 1000, 9999 }) {
      var mono = MakeTone(n, 30, 9000);
      var parsed = new HpsReader().Read(new HpsWriter().Write([mono], 16000));
      Assert.That(parsed.Info.SampleCount, Is.EqualTo(n), $"sampleCount mismatch for n={n}");
      Assert.That(parsed.Pcm[0].Length, Is.EqualTo(n));
    }
  }

  [Test]
  public void Descriptor_ListsFullAndPerChannelAndMetadata() {
    var blob = new HpsWriter().Write([MakeTone(5000, 40, 10000), MakeTone(5000, 60, 8000)], 22050);
    using var ms = new MemoryStream(blob);
    var entries = new HpsFormatDescriptor().List(ms, null);

    Assert.That(entries.Any(e => e.Name == "FULL.hps" && e.Kind == "Container"), Is.True);
  ...
  public void Descriptor_Metadata_DescribesStream() {
    var blob = new HpsWriter().Write([MakeTone(1000, 25, 8000)], 48000);
    using var ms = new MemoryStream(blob);
    using var output = new MemoryStream();
    new HpsFormatDescriptor().ExtractEntry(ms, "metadata.ini", output, null);
  ...
  public void Descriptor_FullOnlyFallback_OnGarbage() {
    var blob = " HALPST\0"u8.ToArray().Concat(new byte[8]).ToArray(); // header with no channel info
    using var ms = new MemoryStream(blob);
    var entries = new HpsFormatDescriptor().List(ms, null);
    Assert.That(entries.Count, Is.EqualTo(1));
    Assert.That(entries[0].Name, Is.EqualTo("FULL.hps"));
  }

  [Test]
  public void Create_FromPerChannelWavs_ProducesReadableHps() {
    ...
    using var output = new MemoryStream();
    new HpsFormatDescriptor().Create(output, inputs, new Compression.Registry.FormatCreateOptions());
    var parsed = new HpsReader().Read(output.ToArray());
    Assert.That(parsed.Info.NumChannels, Is.EqualTo(2));
  ...
  public void Create_FullPassthrough_ReturnsVerbatim() {
    var original = new HpsWriter().Write([MakeTone(2000, 30, 8000)], 16000);
    var inputs = new List<Compression.Registry.ArchiveInputInfo> {
      Compression.Registry.ArchiveInputInfo.InMemory("FULL.hps", original),
    };
    using var output = new MemoryStream();
    new HpsFormatDescriptor().Create(output, inputs, new Compression.Registry.FormatCreateOptions());
