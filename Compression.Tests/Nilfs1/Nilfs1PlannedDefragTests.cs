...
/// <summary>
/// Nilfs1 lays a volume out again by moving payloads inside the one area where
...
/// </remarks>
[TestFixture]
public class Nilfs1PlannedDefragTests {
  ...
  private static MemoryStream Volume(out Dictionary<string, byte[]> kept) {
    var work = Path.Combine(Path.GetTempPath(), "cwb_Nilfs1_" + Guid.NewGuid().ToString("N")[..8]);
    ...
    try {
      ...
      var descriptor = new Nilfs1FormatDescriptor();
  ...
  public void Defragment_KeepsEveryPayloadAndTheVolumesSize(DefragMode mode) {
    ...
    image.Position = 0;
    new Nilfs1FormatDescriptor().Defragment(image, new DefragOptions { Mode = mode });
    Assert.That(image.Length, Is.EqualTo(size), "a volume keeps its size");

    image.Position = 0;
    using var reader = new Nilfs1Reader(image);
