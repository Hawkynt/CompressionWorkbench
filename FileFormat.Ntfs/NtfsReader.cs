...
public sealed class NtfsReader : IDisposable {
  ...
  private void Parse() {
    ...
    // Read MFT records (at minimum records 0-15)
    var mftOffset = _mftCluster * _clusterSize;
    var maxRecords = 16;

    // Try to read more records if space allows
    var mftAreaSize = _data.Length - mftOffset;
    if (mftAreaSize > 0) {
      var possibleRecords = (int)(mftAreaSize / _mftRecordSize);
      if (possibleRecords > maxRecords)
        maxRecords = possibleRecords;
    }
