...
public partial class AmrWb
{
    ...
    private const int ORDER = 16;            /* order of linear prediction filter */
    //private const int ISF_GAP = 128;         /* 50 Hz */
    private const int N_SURV = 4;

    private const int SIZE_BK1 = 256;
    private const int SIZE_BK2 = 256;
    private const int SIZE_BK21 = 64;
    private const int SIZE_BK22 = 128;
    private const int SIZE_BK23 = 128;
    private const int SIZE_BK24 = 32;
    private const int SIZE_BK25 = 32;

    private const int SIZE_BK21_36b = 128;
    private const int SIZE_BK22_36b = 128;
    private const int SIZE_BK23_36b = 64;

    /* means of ISFs */
    private static short[] mean_isf = new short [ORDER]
    {
    ...
    private static short[] dico1_isf = new short[SIZE_BK1 * 9]
    {
    ...
    private static short[] dico2_isf = new short[SIZE_BK2 * 7]
    {
    ...
    private static short[] dico21_isf = new short[SIZE_BK21 * 3]
    {
       329,   409,   249,
       -33,   505,   160,
       -29,   -14,   582,
    ...
    private static short[] dico22_isf = new short[SIZE_BK22 * 3]
    {
      -127,   310,    42,
      -242,   197,     5,
      -151,    84,   -17,
    ...
    private static short[] dico23_isf = new short[SIZE_BK23 * 3]
    {
       -10,   151,   359,
       136,   298,   223,
       255,  -104,   290,
    ...
    private static short[] dico24_isf = new short[SIZE_BK24 * 3]
    {
       -79,   -89,    -4,
      -171,    77,  -211,
       160,  -193,    98,
    ...
    private static short[] dico25_isf = new short[SIZE_BK25 * 4]
    {
       169,   142,  -119,   115,
       206,   -20,    94,   226,
      -106,   313,   -21,    16,
    ...
    private static short[] dico21_isf_36b = new short[SIZE_BK21_36b * 5]
    {
       -52,   -96,   212,   315,   -73,
        82,  -204,   363,   136,  -197,
      -126,  -331,   183,   218,   143,
    ...
    private static short[] dico22_isf_36b = new short[SIZE_BK22_36b * 4]
    {
      -298,    -6,    95,    31,
      -213,   -87,  -122,   261,
         4,   -49,   208,    14,
    ...
    private static short[] dico23_isf_36b = new short[SIZE_BK23_36b * 7]
    {
    ...
    private const int MU = 10923;                   /* Prediction factor   (1.0/3.0) in Q15 */
    private const int N_SURV_MAX = 4;               /* 4 survivors max */
    private const int ALPHA = 29491;                   /* 0. 9 in Q15     */
    private const int ONE_ALPHA = (32768 - ALPHA);            /* (1.0 - ALPHA) in Q15 */
    ...
    private unsafe void Qpisf_2s_46b(
         short[] isf1,                       /* (i) Q15 : ISF in the frequency domain (0..0.5) */
         short[] isf_q,                       /* (o) Q15 : quantized ISF               (0..0.5) */
         short[] past_isfq,                   /* (io)Q15 : past ISF quantizer                   */
         short[] indice,                      /* (o)     : quantization indices                 */
         short nb_surv)                      /* (i)     : number of survivor (1, 2, 3 or 4)    */
    {
        short i, k;
        short[] tmp_ind = new short[5];
        short[] surv1 = new short[N_SURV_MAX];              /* indices of survivors from 1st stage */
        int temp, min_err, distance;
        short[] isf = new short[ORDER];
        short[] isf_stage2 = new short[ORDER];
        min_err = 0;
        ...
        VQ_stage1(isf, 0, dico1_isf, 0, 9, SIZE_BK1, surv1, nb_surv);
        ...
        for (k = 0; k < nb_surv; k++)
        {
            ...
            tmp_ind[0] = Sub_VQ(isf_stage2, 0, dico21_isf, 3, SIZE_BK21, ref min_err);
            temp = min_err;
            tmp_ind[1] = Sub_VQ(isf_stage2, 3, dico22_isf, 3, SIZE_BK22, ref min_err);
            temp = L_add(temp, min_err);
            tmp_ind[2] = Sub_VQ(isf_stage2, 6, dico23_isf, 3, SIZE_BK23, ref min_err);
            temp = L_add(temp, min_err);
        ...
        VQ_stage1(isf, 9, dico2_isf, 0, 7, SIZE_BK2, surv1, nb_surv);
        ...
        for (k = 0; k < nb_surv; k++)
        {
            ...
            tmp_ind[0] = Sub_VQ(isf_stage2, 0, dico24_isf, 3, SIZE_BK24, ref min_err);
            temp = min_err;
            tmp_ind[1] = Sub_VQ(isf_stage2, 3, dico25_isf, 4, SIZE_BK25, ref min_err);
            temp = L_add(temp, min_err);
    ...
    )
    {
        ...
        for (k = 0; k < nb_surv; k++)
        {
            ...
            tmp_ind[0] = Sub_VQ(isf_stage2, 0, dico21_isf_36b, 5, SIZE_BK21_36b, ref min_err);
            temp = min_err;
            tmp_ind[1] = Sub_VQ(isf_stage2, 5, dico22_isf_36b, 4, SIZE_BK22_36b, ref min_err);
            temp = L_add(temp, min_err);
        ...
        for (k = 0; k < nb_surv; k++)
        {
            ...
            tmp_ind[0] = Sub_VQ(isf_stage2, 0, dico23_isf_36b, 7, SIZE_BK23_36b, ref min_err);
            temp = min_err;
    ...
    private void Dpisf_2s_46b(
         short[] indice,                      /* input:  quantization indices                       */
         short[] isf_q,                       /* output: quantized ISF in frequency domain (0..0.5) */
         short[] past_isfq,                   /* i/0   : past ISF quantizer                    */
         short[] isfold,                      /* input : past quantized ISF                    */
         short[] isf_buf,                     /* input : isf buffer                                                        */
         short bfi,                           /* input : Bad frame indicator                   */
         short enc_dec
    )
    {
        short[] ref_isf = new short[M];
        short i, j, tmp;
        int L_tmp;
    ...
    void Reorder_isf(
         short[] isf,                         /* (i/o) Q15: ISF in the frequency domain (0..0.5) */
         short min_dist,                      /* (i) Q15  : minimum distance to keep             */
         short n                              /* (i)      : number of ISF                        */
    )
    {
        short i, isf_min;
    ...
    short Sub_VQ(                             /* output: return quantization index     */
         short[] x,                           /* input : ISF residual vector           */
         int x_index,
         short[] dico,                        /* input : quantization codebook         */
         short dim,                           /* input : dimention of vector           */
         short dico_size,                     /* input : size of quantization codebook */
         ref int distance                     /* output: error of quantization         */
    )
    {
        short i, j, index, temp;
        //short* p_dico;
        int dico_index = 0;
        int dist_min, dist;
        ...
        for (i = 0; i < dico_size; i++)
        {
            ...
            {
                //temp = sub(x[j], *p_dico++);
                temp = sub(x[j + x_index], dico[dico_index++]);
        ...
        dico_index = index * dim;
        for (j = 0; j < dim; j++)
        {
            x[j + x_index] = dico[dico_index++];
        }

        return index;
    }

    private void VQ_stage1(
         short[] x,                           /* input : ISF residual vector           */
         int x_index,                         // index into x
         short[] dico,                        /* input : quantization codebook         */
         int dico_index,                      // index into dico
         short dim,                           /* input : dimention of vector           */
         short dico_size,                     /* input : size of quantization codebook */
         short[] index,                       /* output: indices of survivors          */
         short surv)                          /* input : number of survivor            */
    {
        short i, j, k, l, temp;

        //short* p_dico;
        int[] dist_min = new int[N_SURV_MAX];
        int dist;

        for (i = 0; i < surv; i++)
        {
            dist_min[i] = int.MaxValue;
            index[i] = i;
        }
