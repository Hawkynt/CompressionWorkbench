...
public partial class AmrWb
{
    ...
    private int quant_1p_N1(                        /* (o) return N+1 bits             */
         short pos,                           /* (i) position of the pulse       */
         short N)                             /* (i) number of bits for position */
    {
        short mask;
        int index;
         ...
         *-------------------------------------------------------*/
        index = L_deposit_l((short)(pos & mask));
    ...
    private unsafe void dec_1p_N1(int index, short N, short offset, short* pos, short NB_POS)
    {
        short pos1;
        int mask, i;
    ...
    private int quant_2p_2N1(                       /* (o) return (2*N)+1 bits         */
         short pos1,                          /* (i) position of the pulse 1     */
         short pos2,                          /* (i) position of the pulse 2     */
         short N)                             /* (i) number of bits for position */
    {
        short mask, tmp;
        int index;
        ...
        if (((pos2 ^ pos1) & NB_POS) == 0)
        {
            ...
            if (sub(pos1, pos2) <= 0)          /* ((pos1 - pos2) <= 0) */
            {
                ...
                index = L_deposit_l(add(shl(((short)(pos1 & mask)), N), ((short)(pos2 & mask))));
            } else
            {
                /* ((pos2 & mask) << N) + (pos1 & mask); */
                index = L_deposit_l(add(shl(((short)(pos2 & mask)), N), ((short)(pos1 & mask))));
            }
        ...
        {
            /* sign of 1st pulse != sign of 2th pulse */
            if (sub((short)(pos1 & mask), (short)(pos2 & mask)) <= 0)
            {
    ...
    private unsafe void dec_2p_2N1(int index, short N, short offset, short* pos, short NB_POS)
    {
        short pos1, pos2, tmp;
        int mask, i;
        ...
        tmp = shl(N, 1);
        i = (L_shr(index, tmp) & 1); /* i = (index >> (2*N)) & 1; */
    ...
    private int quant_3p_3N1(                       /* (o) return (3*N)+1 bits         */
         short pos1,                          /* (i) position of the pulse 1     */
         short pos2,                          /* (i) position of the pulse 2     */
         short pos3,                          /* (i) position of the pulse 3     */
         short N)                             /* (i) number of bits for position */
    {
        short nb_pos;
        int index;
        ...
        if (((pos1 ^ pos2) & nb_pos) == 0)
        {
            ...
            /* index += (pos1 & nb_pos) << N; */
            index = L_add(index, L_shl(L_deposit_l((short)(pos1 & nb_pos)), N));
        ...
        } else
        {
            ...
            /* index += (pos2 & nb_pos) << N;       */
            index = L_add(index, L_shl(L_deposit_l((short)(pos2 & nb_pos)), N));
    ...
    private unsafe void dec_3p_3N1(int index, short N, short offset, short* pos, short NB_POS)
    {
        short j, tmp;
        int mask, idx;
        ...
        dec_2p_2N1(idx, (short)(N - 1), j, pos, NB_POS);
        ...
        dec_1p_N1(idx, N, offset, &pos[2], NB_POS);

        return;
    }

    int quant_4p_4N1(                       /* (o) return (4*N)+1 bits         */
         short pos1,                          /* (i) position of the pulse 1     */
         short pos2,                          /* (i) position of the pulse 2     */
         short pos3,                          /* (i) position of the pulse 3     */
         short pos4,                          /* (i) position of the pulse 4     */
         short N)                             /* (i) number of bits for position */
    {
        short nb_pos;
        int index;
    ...
    private unsafe void dec_4p_4N1(int index, short N, short offset, short* pos, short NB_POS)
    {
        short j, tmp;
        int mask, idx;
        ...
        dec_2p_2N1(idx, N, offset, &pos[2], NB_POS);   /* dec_2p_2N1(idx, N, offset, pos+2); */

        return;
    }

    private unsafe int quant_4p_4N(                        /* (o) return 4*N bits             */
         short* pos,                         /* (i) position of the pulse 1..4  */
         short N)                             /* (i) number of bits for position */
    {
        short i, j, k, nb_pos, mask, n_1, tmp;
        //short posA[4], posB[4];
        short[] posA = new short[4];
        short[] posB = new short[4];
        ...
        index = L_add(index, L_shl((L_deposit_l(i) & (3)), tmp));

        return (index);
    }

    private unsafe void dec_4p_4N(int index, short N, short offset, short* pos, short NB_POS)
    {
        short j, n_1, tmp;
        ...
        {                                      /* ((index >> ((4*N)-2)) & 3) */
            case 0:
                ...
                {                                  /* (((index >> ((4*n_1)+1)) & 1) == 0) */
                    dec_4p_4N1(index, n_1, offset, pos, NB_POS);
                } else
                {
                    dec_4p_4N1(index, n_1, j, pos, NB_POS);
                }
                break;
            case 1:
                tmp = add(extract_l(L_shr(L_mult(3, n_1), 1)), 1); /* dec_1p_N1((index>>((3*n_1)+1)), n_1, offset, pos) */
                dec_1p_N1(L_shr(index, tmp), n_1, offset, pos, NB_POS);
                //dec_3p_3N1(index, n_1, j, pos + 1, NB_POS); 
                dec_3p_3N1(index, n_1, j, &pos[1], NB_POS);
                break;
            case 2:
                tmp = add(shl(n_1, 1), 1);         /* dec_2p_2N1((index>>((2*n_1)+1)), n_1, offset, pos); */
                dec_2p_2N1(L_shr(index, tmp), n_1, offset, pos, NB_POS);
                //dec_2p_2N1(index, n_1, j, pos + 2, NB_POS);
                dec_2p_2N1(index, n_1, j, &pos[2], NB_POS);
                break;
            case 3:
                tmp = add(n_1, 1);                 /* dec_3p_3N1((index>>(n_1+1)), n_1, offset, pos); */
                dec_3p_3N1(L_shr(index, tmp), n_1, offset, pos, NB_POS);
                //dec_1p_N1(index, n_1, j, pos + 3, NB_POS);
                dec_1p_N1(index, n_1, j, &pos[3], NB_POS);
                break;
        }
        return;
    ...
    private unsafe int quant_5p_5N(                        /* (o) return 5*N bits             */
         short* pos,                         /* (i) position of the pulse 1..5  */
         short N)                             /* (i) number of bits for position */
    {
        short i, j, k, nb_pos, n_1, tmp;
        //short posA[5], posB[5];
        short[] posA = new short[5];
        short[] posB = new short[5];
        ...
        switch (i)
        {
            case 0:
                ...
                index = L_shl(1, tmp);   /* index = 1 << ((5*N)-1); */
    ...
    private unsafe void dec_5p_5N(int index, short N, short offset, short* pos, short NB_POS)
    {
        short j, n_1, tmp;
        int idx;
        ...
        {
            dec_3p_3N1(idx, n_1, offset, pos, NB_POS);
            //dec_2p_2N1(index, N, offset, pos + 3);
            dec_2p_2N1(index, N, offset, &pos[3], NB_POS);
        }
        else
        {
            dec_3p_3N1(idx, n_1, j, pos, NB_POS);
            //dec_2p_2N1(index, N, offset, pos + 3);
            dec_2p_2N1(index, N, offset, &pos[3], NB_POS);
        }
        return;
    ...
    private unsafe int quant_6p_6N_2(                      /* (o) return (6*N)-2 bits         */
         short* pos,                         /* (i) position of the pulse 1..6  */
         short N)                             /* (i) number of bits for position */
    {
        short i, j, k, nb_pos, n_1;
        //short posA[6], posB[6];
        short* posA = stackalloc short[6];
        short* posB = stackalloc short[6];
        ...
        {
            case 0:
                index = L_shl(1, (short)(6 * N - 5));        /* index = 1 << ((6*N)-5); */
            ...
            case 2:
                ...
                index = L_add(index, L_shl(quant_4p_4N(posB, n_1), (short)(2 * n_1 + 1)));
                index = L_add(index, quant_2p_2N1(posA[0], posA[1], n_1));      /* index += quant_2p_2N1(posA[0], posA[1], n_1); */
                break;
            case 3:
                index = L_shl(quant_3p_3N1(posA[0], posA[1], posA[2], n_1), (short)(3 * n_1 + 1));    /* index = quant_3p_3N1(posA[0], posA[1], posA[2], n_1) << ((3*n_1)+1); */
                index = L_add(index, quant_3p_3N1(posB[0], posB[1], posB[2], n_1));     /* index += quant_3p_3N1(posB[0], posB[1], posB[2], n_1); */
                break;
            case 4:
                i = 2; 
                index = L_shl(quant_4p_4N(posA, n_1), (short)(2 * n_1 + 1));  /* index = quant_4p_4N(posA, n_1) << ((2*n_1)+1); */
        ...
        index = L_add(index, L_shl((L_deposit_l(i) & 3), (short)(6 * N - 4))); /* index += (i & 3) << ((6*N)-4); */

        return (index);
    }

    private unsafe void dec_6p_6N_2(int index, short N, short offset, short* pos, short NB_POS)
    {
        ...
        offsetA = offsetB = j;  
        if ((L_shr(index, (short)(6 * N - 5)) & 1L) == 0)
        {                                      /* if (((index >> ((6*N)-5)) & 1) == 0) */
            offsetA = offset; 
        } else
        {
        ...
        switch (L_shr(index, (short)(6 * N - 4)) & 3)
        {                                      /* (index >> ((6*N)-4)) & 3 */
            case 0:
                dec_5p_5N(L_shr(index, N), n_1, offsetA, pos, NB_POS);  /* dec_5p_5N(index>>N, n_1, offsetA, pos); */
                //dec_1p_N1(index, n_1, offsetA, pos + 5);
                dec_1p_N1(index, n_1, offsetA, &pos[5], NB_POS);
                break;
            case 1:
                dec_5p_5N(L_shr(index, N), n_1, offsetA, pos, NB_POS);  /* dec_5p_5N(index>>N, n_1, offsetA, pos); */
                //dec_1p_N1(index, n_1, offsetB, pos + 5);
                dec_1p_N1(index, n_1, offsetB, &pos[5], NB_POS);
                break;
            case 2:
                dec_4p_4N(L_shr(index, (short)(2 * n_1 + 1)), n_1, offsetA, pos, NB_POS); /* dec_4p_4N(index>>((2*n_1)+1 ), n_1, offsetA, pos); */
                //dec_2p_2N1(index, n_1, offsetB, pos + 4);
                dec_2p_2N1(index, n_1, offsetB, &pos[4], NB_POS);
                break;
            case 3:
                dec_3p_3N1(L_shr(index, (short)(3 * n_1 + 1)), n_1, offset, pos, NB_POS); /* dec_3p_3N1(index>>((3*n_1)+ 1), n_1, offset, pos); */
                //dec_3p_3N1(index, n_1, j, pos + 3);
                dec_3p_3N1(index, n_1, j, &pos[3], NB_POS);
                break;
        }
        return;
    }
