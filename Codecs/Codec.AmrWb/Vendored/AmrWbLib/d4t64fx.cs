...
public partial class AmrWb
{
    ...
    private unsafe void DEC_ACELP_4t64_fx(
         short[] index,                       /* (i) : index (20): 5+5+5+5 = 20 bits.                 */
...
)
    {
        ...
        if (sub(nbbits, 20) == 0)
        {
            ...
            {
                L_index = index[k];
                dec_1p_N1(L_index, 4, 0, pos, NB_POS);
        ...
        else if (sub(nbbits, 36) == 0)
        {
            ...
            {
                L_index = index[k];
                dec_2p_2N1(L_index, 4, 0, pos, NB_POS);
        ...
        else if (sub(nbbits, 44) == 0)
        {
            ...
            {
                L_index = index[k];
                dec_3p_3N1(L_index, 4, 0, pos, NB_POS);
        ...
        else if (sub(nbbits, 64) == 0)
        {
            for (k = 0; k < NB_TRACK; k++)
            {
                ...
                dec_4p_4N(L_index, 4, 0, pos, NB_POS);
        ...
        else if (sub(nbbits, 72) == 0)
        {
            for (k = 0; k < NB_TRACK - 2; k++)
            {
                ...
                dec_5p_5N(L_index, 4, 0, pos, NB_POS);
        ...
        else if (sub(nbbits, 88) == 0)
        {
            for (k = 0; k < NB_TRACK; k++)
            {
                ...
                dec_6p_6N_2(L_index, 4, 0, pos, NB_POS);
    ...
    private unsafe void add_pulses(short* pos, short nb_pulse, short track, short* code)
    {
        short i, k;
        ...
        for (k = 0; k < nb_pulse; k++)
        {
            ...
            i = (short)(add(shl((short)(pos[k] & (NB_POS - 1)), 2), track));
            if ((pos[k] & NB_POS) == 0)
