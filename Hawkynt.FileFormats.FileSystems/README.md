...
| [NTFS](https://en.wikipedia.org/wiki/NTFS) | `Ntfs` | R/W | ✅ | ✅ moving | ✅ | ✅ | ✅ | ✅ | [`ntfsls` / `ntfsinfo` / `ntfsfix`](https://github.com/tuxera/ntfs-3g) (ntfs-3g) + reverse `mkfs.ntfs` | Writes either FILE record header layout — NTFS 3.1 (record number at 44, USA at 48) or pre-3.1 (no record number, USA at 42) — selected by the `NtfsVersion` create option, and reads both. Edits relayout the volume; `ntfs3` mounts it but lists an empty root (root `$I30` index pending) | [MS-FSCC](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-fscc/) |
