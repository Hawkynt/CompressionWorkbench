...
| [ReFS](https://en.wikipedia.org/wiki/ReFS) | `Refs` | R/W | ✅ moving | ✅ | — | ✅ | own reader + struct-parity tests | ReFS 3.x; mutation is offline-quiescent only (see limitations). The Layout tick covers analysis only — there is no creator to re-lay a volume with | [ReFS overview](https://learn.microsoft.com/windows-server/storage/refs/refs-overview) |
...
| [eCryptfs](https://en.wikipedia.org/wiki/ECryptfs) | `Ecryptfs` | R | ✅ rebuild | — | — | — | detection of the on-disk signature | Payload is encrypted; plaintext needs the FEK. The Defrag tick is not honoured — the verb refuses, and the marker comes out with the operation-coverage rewrite | [eCryptfs](https://www.kernel.org/doc/html/latest/filesystems/ecryptfs.html) |
...
| [Tahoe-LAFS share](https://en.wikipedia.org/wiki/Tahoe-LAFS) | `TahoeLafs` | R | ✅ rebuild | — | — | — | detection of the on-disk signature | Shares are capability-encrypted; no read-cap, no plaintext. The Defrag tick is not honoured — the verb refuses, and the marker comes out with the operation-coverage rewrite | [Tahoe-LAFS](https://tahoe-lafs.org/) |
