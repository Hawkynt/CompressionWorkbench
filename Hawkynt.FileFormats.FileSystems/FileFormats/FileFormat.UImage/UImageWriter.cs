...
public static class UImageWriter {
  ...
  /// <summary>Copies the editable fields from an already parsed legacy header.</summary>
  public static Header From(UImageReader.LegacyHeader header) => new(
    header.Timestamp,
    header.LoadAddress,
    header.EntryPoint,
    header.Os,
  ...
  /// <summary>Copies the editable fields from an already parsed uImage.</summary>
  public static Header From(UImageReader.UImage image) => new(
    image.Timestamp,
    image.LoadAddress,
    image.EntryPoint,
    image.Os,
  ...
  /// <summary>
  /// Reads editable fields from a raw 64-byte uImage header. Size and CRC fields
  /// are deliberately not retained because <see cref="Write"/> derives them from
  /// the new payload.
  /// </summary>
  public static Header FromHeaderBytes(ReadOnlySpan<byte> bytes)
    => From(UImageReader.ReadHeader(bytes));

  /// <summary>
  /// Applies fields from the descriptor's rendered <c>metadata.ini</c> to an
  /// existing header model. Unknown and derived fields are ignored.
  /// </summary>
  public static Header ApplyMetadata(Header header, ReadOnlySpan<byte> metadata) {
  ...
  public static void Write(Stream output, Header header, ReadOnlySpan<byte> body) {
    ...
    // U-Boot's legacy field is exactly IH_NMLEN bytes. strncpy-style writing
    // NUL-pads shorter names but a full 32-byte name is valid without a terminator.
    var name = Encoding.ASCII.GetBytes(header.Name ?? "");
    var kept = Math.Min(name.Length, UImageReader.NameLength);
  ...
  public static (Header Header, byte[] Body) From(IReadOnlyList<ArchiveInputInfo> inputs) {
    ...
    foreach (var input in inputs) {
      ...
      if (leaf.Equals(MetadataName, StringComparison.OrdinalIgnoreCase)) {
        header = ApplyMetadata(header, input.ReadContent());
        continue;
      }
      ...
      // the only compression this project writes inline.
