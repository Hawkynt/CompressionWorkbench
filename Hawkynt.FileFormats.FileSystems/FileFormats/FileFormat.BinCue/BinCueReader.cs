...
/// <summary>
/// Reads the ISO 9660 file system embedded in a BIN/CUE CD-ROM disc image.
/// Supports raw 2352-byte sectors (Mode 1 and Mode 2 Form 1), 2336-byte sectors,
/// and plain 2048-byte ISO sector streams.
/// </summary>
public sealed class BinCueReader : IDisposable {
  // ISO 9660 constants
  private const int Iso9660SectorSize = 2048;
  private const int RawSectorSize = 2352;
  private const int SectorSize2336 = 2336;
  private const int PvdLba = 16;
  private const int Mode1DataOffset = 16;   // sync(12) + addr(3) + mode(1) = 16
  private const int Mode2Form1DataOffset = 24; // sync(12) + addr(3) + mode(1) + subheader(8) = 24
  ...
  public IReadOnlyList<BinCueEntry> Entries { get; }

  /// <summary>
  /// Initializes a new <see cref="BinCueReader"/> from a BIN stream.
  /// </summary>
  /// <param name="stream">The stream containing BIN sector data.</param>
  /// <param name="leaveOpen">Whether to leave the stream open on dispose.</param>
  public BinCueReader(Stream stream, bool leaveOpen = false) {
    this._stream = stream ?? throw new ArgumentNullException(nameof(stream));
    this._leaveOpen = leaveOpen;

    (this._sectorSize, this._dataOffset) = DetectSectorGeometry(stream);

    var entries = new List<BinCueEntry>();
  ...
  public byte[] Extract(BinCueEntry entry) {
  ...
  private static (int SectorSize, int DataOffset) DetectSectorGeometry(Stream stream) {
    // Try 2352-byte raw sectors first: look for CD001 at LBA 16, Mode 1 offset 16
    if (TryProbe(stream, RawSectorSize, Mode1DataOffset))
      return (RawSectorSize, Mode1DataOffset);

    // Try 2352-byte raw sectors, Mode 2 Form 1 offset 24
    if (TryProbe(stream, RawSectorSize, Mode2Form1DataOffset))
      return (RawSectorSize, Mode2Form1DataOffset);

    // Try 2336-byte sectors (sync stripped, data at offset 8 = addr+mode+subheader)
    if (TryProbe(stream, SectorSize2336, 8))
      return (SectorSize2336, 8);

    // Try plain 2048-byte ISO sectors (cooked)
    if (TryProbe(stream, Iso9660SectorSize, 0))
      return (Iso9660SectorSize, 0);

    // Fall back to raw 2352 / Mode 1 even if probe failed (image may still be valid)
    return (RawSectorSize, Mode1DataOffset);
  }
  ...
  private void TryParseIso9660(List<BinCueEntry> entries) {
    // Read PVD at LBA 16
  ...
  private void WalkDirectory(int dirLba, int dirSize, string parentPath, List<BinCueEntry> entries) {
    ...
    while (bytesRead < dirSize) {
      ...
      // Zero-length record signals end of directory records in this sector
      if (recordLen == 0) {
      ...
      if (bufOffset + recordLen > Iso9660SectorSize) {
        // Record spans sector boundary — advance to next sector
        var remaining = Iso9660SectorSize - bufOffset;
        bytesRead += remaining;
        bufOffset = Iso9660SectorSize;
        continue;
  ...
  private void ParseDirectoryRecord(ReadOnlySpan<byte> record, string parentPath, List<BinCueEntry> entries) {
    if (record.Length < 34)
      return;
    ...
    // ISO 9660 file identifiers may contain ";1" version suffix — strip it
  ...
  /// <summary>
  /// Reads the user-data portion of a single sector at the given LBA.
  /// Returns null if the read fails or the LBA is out of range.
  /// </summary>
