...
internal sealed class CxfsV4Writer {
  ...
  public void WriteTo(Stream output) {
    ...
    ValidateShortFormDirectory(files);
    ...
    var usedInodes = 3 + files.Length; // root + rbm + rsum + regular files
    ...
    WriteRootDirectory(image.AsSpan(InodeOffset(0), InodeSize), files);
    ...
    for (var i = 0; i < files.Length; ++i) {
      var inode = image.AsSpan(InodeOffset(3 + i), InodeSize);
  ...
  private static string NormalizeName(string name) {
    ...
    if (normalized.Contains('/'))
      throw new NotSupportedException("CXFS v4 mutation profile currently supports root-level regular files only.");
    var bytes = Encoding.UTF8.GetByteCount(normalized);
    if (bytes is <= 0 or > 255)
      throw new NotSupportedException("CXFS v4 file names must be 1..255 UTF-8 bytes.");
    return normalized;
  }

  private static void ValidateShortFormDirectory(IReadOnlyList<(string Name, byte[] Data)> files) {
    if (files.Count > InodesPerChunk - 3)
      throw new NotSupportedException("CXFS v4 mutation profile is limited to one 64-inode chunk.");
    var size = 6;
    foreach (var (name, _) in files)
      size += 7 + Encoding.UTF8.GetByteCount(name);
    if (size > ShortFormCapacity)
      throw new NotSupportedException(
        $"CXFS v4 mutation profile requires the root directory to remain short-form ({size} > {ShortFormCapacity} bytes).");
  }
  ...
  private static void WriteRootDirectory(Span<byte> inode, IReadOnlyList<(string Name, byte[] Data)> files) {
    WriteInodeCoreV2(inode, mode: 0x41ED, format: 1, nlink: 2);
    var fork = inode[ForkOffset..];
    fork[0] = (byte)files.Count;
    fork[1] = 0;
    BinaryPrimitives.WriteUInt32BigEndian(fork[2..], (uint)RootIno);

    var pos = 6;
    ushort dataOffset = 0x30;
    for (var i = 0; i < files.Count; ++i) {
      var name = Encoding.UTF8.GetBytes(files[i].Name);
      fork[pos] = (byte)name.Length;
      BinaryPrimitives.WriteUInt16BigEndian(fork[(pos + 1)..], dataOffset);
      name.CopyTo(fork[(pos + 3)..]);
      BinaryPrimitives.WriteUInt32BigEndian(fork[(pos + 3 + name.Length)..], (uint)(RootIno + (ulong)(3 + i)));
      pos += 7 + name.Length;
