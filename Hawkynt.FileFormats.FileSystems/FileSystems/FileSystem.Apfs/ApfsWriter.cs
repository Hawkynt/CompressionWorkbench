...
public sealed class ApfsWriter {
  ...
  private readonly record struct FileEntry(string Name, long Size, byte[]? Data, Func<Stream>? Opener, DateTime? LastModified);
    ...
    foreach (var node in tree.Nodes) {
      ...
      list.Add((node.Ino, APFS_TYPE_INODE, string.Empty,
        ...
        BuildInodeValue(node.Ino, parentId: node.ParentIno, size: size,
          ...
          name: node.Name, lastModified: node.Payload?.LastModified)));
  ...
  private static byte[] BuildInodeValue(ulong ino, ulong parentId, long size, bool isDir, uint nchildren,
    ...
    => ApfsInodeRecord.BuildValue(ino, parentId, size, isDir, nchildren, internalFlags, name, lastModified);
