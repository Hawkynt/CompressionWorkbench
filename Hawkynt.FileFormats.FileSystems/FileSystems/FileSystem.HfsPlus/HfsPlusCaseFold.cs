...
/// <summary>
/// Implements the HFS+ catalog key name comparison for case-folding volumes
/// (<c>keyCompareType = kHFSCaseFolding</c>) as specified in Apple TN1150
/// ("HFS Plus Volume Format"), section "Unicode Subtleties". HFS+ catalog
/// records must be sorted, and remain sorted, by this exact ordering — a
/// volume whose keys are not monotonic under the declared compare type is
/// rejected by <c>fsck_hfs</c> with "Keys out of order".
/// <para>
/// The comparison folds each UTF-16 code unit to a canonical (roughly
/// lower-case) value through a two-level table: the first 256 entries map a
/// code unit's high byte to the offset of its 256-entry sub-table, or zero
/// when that block has no folding and no ignorable characters. Code units
/// that fold to zero are ignored entirely (they do not participate in the
/// comparison), which is how combining marks and other zero-width characters
/// are handled. The NUL code unit folds to 0xFFFF so a name may legitimately
/// contain it while zero stays reserved as the ignore sentinel.
/// </para>
/// </summary>
internal static class HfsPlusCaseFold {

  // Case-folding for the Basic Latin block (code units 0x00..0xFF). TN1150
  // keeps this range in its own flat table so the common case skips the
  // two-level lookup; it folds A..Z (and the Latin-1 upper-case letters) to
  // their lower-case forms and maps NUL to 0xFFFF so the zero value can stay
  // reserved as the "ignore this code unit" sentinel.
  private static readonly ushort[] LatinCaseFold = {
  ...
  // TN1150 case-folding table: 256 high-byte offsets followed by the
  // 256-entry sub-tables they reference. A high-byte offset of zero means
  // "no folding and no ignorables in this block", so the code unit folds to
  // itself; otherwise the offset is the flat array index of the sub-table,
  // and the low byte selects the entry — exactly the lookup
  // `table[table[c >> 8] + (c & 0xFF)]`.
  private static readonly ushort[] LowerCaseTable = {
    0x0000, 0x0100, 0x0000, 0x0200, 0x0300, 0x0400, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0500, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0600, 0x0700, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    ...
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
    0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0800, 0x0900,
  ...
  /// <summary>
  /// Folds a single UTF-16 code unit to its case-folded comparison value.
  /// Returns zero for ignorable code units (the caller skips those).
  /// </summary>
  private static ushort Fold(ushort c) {
    // Basic Latin uses its own flat fold table; everything else goes through
    // the two-level high-byte/sub-table lookup.
    if (c < 0x0100)
      return LatinCaseFold[c];
    var page = LowerCaseTable[c >> 8];
    return page == 0 ? c : LowerCaseTable[page + (c & 0x00FF)];
  }

  /// <summary>
  /// Compares two UTF-16 names by the HFS+ case-folding ordering. Returns a
  /// negative value if <paramref name="a"/> sorts first, positive if
  /// <paramref name="b"/> sorts first, and zero if they are equal under the
  /// fold. Ignorable code units (those folding to zero) are skipped on both
  /// sides, exactly as TN1150's FastUnicodeCompare specifies.
  /// </summary>
  public static int Compare(string a, string b) {
    int ia = 0, ib = 0;
    while (true) {
      ushort ca = 0, cb = 0;
      while (ia < a.Length && (ca = Fold(a[ia++])) == 0) { }
      while (ib < b.Length && (cb = Fold(b[ib++])) == 0) { }
      if (ca != cb) return ca < cb ? -1 : 1;
      if (ca == 0) return 0; // both exhausted (only ignorables remained)
    }
  }
}
