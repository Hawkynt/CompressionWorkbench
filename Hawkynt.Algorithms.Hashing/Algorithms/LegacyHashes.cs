using System.Buffers.Binary;
using System.Numerics;

namespace Hawkynt.Algorithms.Hashing;

/// <summary>MD2 from RFC 1319.</summary>
public static class Md2 {
  private static readonly byte[] S = [
    
  ];

  public static byte[] Compute(ReadOnlySpan<byte> data) {
    var pad = 16 - data.Length % 16;
    var padded = new byte[data.Length + pad];
    data.CopyTo(padded);
    padded.AsSpan(data.Length).Fill((byte)pad);

    Span<byte> checksum = stackalloc byte[16];
    checksum.Clear();
    byte l = 0;
    for (var offset = 0; offset < padded.Length; offset += 16) {
      for (var i = 0; i < 16; ++i) {
        var value = padded[offset + i];
        checksum[i] ^= S[value ^ l];
        l = checksum[i];
      }
    }

    var final = new byte[padded.Length + 16];
    padded.CopyTo(final, 0);
    checksum.CopyTo(final.AsSpan(padded.Length));

    Span<byte> x = stackalloc byte[48];
    x.Clear();
    for (var offset = 0; offset < final.Length; offset += 16) {
      for (var i = 0; i < 16; ++i) {
        x[16 + i] = final[offset + i];
        x[32 + i] = (byte)(x[16 + i] ^ x[i]);
      }

      byte t = 0;
      for (var round = 0; round < 18; ++round) {
        for (var i = 0; i < 48; ++i) {
          x[i] ^= S[t];
          t = x[i];
        }
        t = unchecked((byte)(t + round));
      }
    }

    return x[..16].ToArray();
  }
}

/// <summary>MD4 from RFC 1320.</summary>
public static class Md4 {
  public static byte[] Compute(ReadOnlySpan<byte> data) {
    uint a0 = 0x67452301;
    uint b0 = 0xEFCDAB89;
    uint c0 = 0x98BADCFE;
    uint d0 = 0x10325476;

    var paddedLength = checked(((data.Length + 9 + 63) / 64) * 64);
    var buffer = new byte[paddedLength];
    data.CopyTo(buffer);
    buffer[data.Length] = 0x80;
    BinaryPrimitives.WriteUInt64LittleEndian(buffer.AsSpan(paddedLength - 8), (ulong)data.Length * 8);

    Span<uint> x = stackalloc uint[16];
    for (var offset = 0; offset < buffer.Length; offset += 64) {
      var block = buffer.AsSpan(offset, 64);
      for (var i = 0; i < 16; ++i)
        x[i] = BinaryPrimitives.ReadUInt32LittleEndian(block[(i * 4)..]);

      var a = a0;
      var b = b0;
      var c = c0;
      var d = d0;

      Round1(ref a, b, c, d, x[0], 3); Round1(ref d, a, b, c, x[1], 7); Round1(ref c, d, a, b, x[2], 11); Round1(ref b, c, d, a, x[3], 19);
      Round1(ref a, b, c, d, x[4], 3); Round1(ref d, a, b, c, x[5], 7); Round1(ref c, d, a, b, x[6], 11); Round1(ref b, c, d, a, x[7], 19);
      Round1(ref a, b, c, d, x[8], 3); Round1(ref d, a, b, c, x[9], 7); Round1(ref c, d, a, b, x[10], 11); Round1(ref b, c, d, a, x[11], 19);
      Round1(ref a, b, c, d, x[12], 3); Round1(ref d, a, b, c, x[13], 7); Round1(ref c, d, a, b, x[14], 11); Round1(ref b, c, d, a, x[15], 19);

      Round2(ref a, b, c, d, x[0], 3); Round2(ref d, a, b, c, x[4], 5); Round2(ref c, d, a, b, x[8], 9); Round2(ref b, c, d, a, x[12], 13);
      Round2(ref a, b, c, d, x[1], 3); Round2(ref d, a, b, c, x[5], 5); Round2(ref c, d, a, b, x[9], 9); Round2(ref b, c, d, a, x[13], 13);
      Round2(ref a, b, c, d, x[2], 3); Round2(ref d, a, b, c, x[6], 5); Round2(ref c, d, a, b, x[10], 9); Round2(ref b, c, d, a, x[14], 13);
      Round2(ref a, b, c, d, x[3], 3); Round2(ref d, a, b, c, x[7], 5); Round2(ref c, d, a, b, x[11], 9); Round2(ref b, c, d, a, x[15], 13);

      Round3(ref a, b, c, d, x[0], 3); Round3(ref d, a, b, c, x[8], 9); Round3(ref c, d, a, b, x[4], 11); Round3(ref b, c, d, a, x[12], 15);
      Round3(ref a, b, c, d, x[2], 3); Round3(ref d, a, b, c, x[10], 9); Round3(ref c, d, a, b, x[6], 11); Round3(ref b, c, d, a, x[14], 15);
      Round3(ref a, b, c, d, x[1], 3); Round3(ref d, a, b, c, x[9], 9); Round3(ref c, d, a, b, x[5], 11); Round3(ref b, c, d, a, x[13], 15);
      Round3(ref a, b, c, d, x[3], 3); Round3(ref d, a, b, c, x[11], 9); Round3(ref c, d, a, b, x[7], 11); Round3(ref b, c, d, a, x[15], 15);

      a0 = unchecked(a0 + a);
      b0 = unchecked(b0 + b);
      c0 = unchecked(c0 + c);
      d0 = unchecked(d0 + d);
    }

    var result = new byte[16];
    BinaryPrimitives.WriteUInt32LittleEndian(result, a0);
    BinaryPrimitives.WriteUInt32LittleEndian(result.AsSpan(4), b0);
    BinaryPrimitives.WriteUInt32LittleEndian(result.AsSpan(8), c0);
    BinaryPrimitives.WriteUInt32LittleEndian(result.AsSpan(12), d0);
    return result;
  }

  private static uint F(uint x, uint y, uint z) => (x & y) | (~x & z);
  private static uint G(uint x, uint y, uint z) => (x & y) | (x & z) | (y & z);
  private static uint H(uint x, uint y, uint z) => x ^ y ^ z;

  private static void Round1(ref uint a, uint b, uint c, uint d, uint x, int s) =>
    a = BitOperations.RotateLeft(unchecked(a + F(b, c, d) + x), s);

  private static void Round2(ref uint a, uint b, uint c, uint d, uint x, int s) =>
    a = BitOperations.RotateLeft(unchecked(a + G(b, c, d) + x + 0x5A827999), s);

  private static void Round3(ref uint a, uint b, uint c, uint d, uint x, int s) =>
    a = BitOperations.RotateLeft(unchecked(a + H(b, c, d) + x + 0x6ED9EBA1), s);
}

/// <summary>SM3 Chinese national cryptographic hash standard (GB/T 32905).</summary>
public static class Sm3 {
  private static readonly uint[] Initial = [
    0x7380166F, 0x4914B2B9, 0x172442D7, 0xDA8A0600,
    0xA96F30BC, 0x163138AA, 0xE38DEE4D, 0xB0FB0E4E
  ];

  public static byte[] Compute(ReadOnlySpan<byte> data) {
    var state = Initial.ToArray();
    var paddedLength = checked(((data.Length + 9 + 63) / 64) * 64);
    var buffer = new byte[paddedLength];
    data.CopyTo(buffer);
    buffer[data.Length] = 0x80;
    BinaryPrimitives.WriteUInt64BigEndian(buffer.AsSpan(paddedLength - 8), (ulong)data.Length * 8);

    Span<uint> w = stackalloc uint[68];
    Span<uint> w1 = stackalloc uint[64];

    for (var offset = 0; offset < buffer.Length; offset += 64) {
      var block = buffer.AsSpan(offset, 64);
      for (var j = 0; j < 16; ++j)
        w[j] = BinaryPrimitives.ReadUInt32BigEndian(block[(j * 4)..]);

      for (var j = 16; j < 68; ++j)
        w[j] = P1(w[j - 16] ^ w[j - 9] ^ BitOperations.RotateLeft(w[j - 3], 15))
             ^ BitOperations.RotateLeft(w[j - 13], 7)
             ^ w[j - 6];

      for (var j = 0; j < 64; ++j)
        w1[j] = w[j] ^ w[j + 4];

      var a = state[0];
      var b = state[1];
      var c = state[2];
      var d = state[3];
      var e = state[4];
      var f = state[5];
      var g = state[6];
      var h = state[7];

      for (var j = 0; j < 64; ++j) {
        var tj = j < 16 ? 0x79CC4519U : 0x7A879D8AU;
        var ss1 = BitOperations.RotateLeft(unchecked(BitOperations.RotateLeft(a, 12) + e + BitOperations.RotateLeft(tj, j)), 7);
        var ss2 = ss1 ^ BitOperations.RotateLeft(a, 12);
        var tt1 = unchecked(Ff(a, b, c, j) + d + ss2 + w1[j]);
        var tt2 = unchecked(Gg(e, f, g, j) + h + ss1 + w[j]);

        d = c;
        c = BitOperations.RotateLeft(b, 9);
        b = a;
        a = tt1;
        h = g;
        g = BitOperations.RotateLeft(f, 19);
        f = e;
        e = P0(tt2);
      }

      state[0] ^= a;
      state[1] ^= b;
      state[2] ^= c;
      state[3] ^= d;
      state[4] ^= e;
      state[5] ^= f;
      state[6] ^= g;
      state[7] ^= h;
    }

    var result = new byte[32];
    for (var i = 0; i < 8; ++i)
      BinaryPrimitives.WriteUInt32BigEndian(result.AsSpan(i * 4), state[i]);
    return result;
  }

  private static uint Ff(uint x, uint y, uint z, int round) => round < 16 ? x ^ y ^ z : (x & y) | (x & z) | (y & z);
  private static uint Gg(uint x, uint y, uint z, int round) => round < 16 ? x ^ y ^ z : (x & y) | (~x & z);
  private static uint P0(uint x) => x ^ BitOperations.RotateLeft(x, 9) ^ BitOperations.RotateLeft(x, 17);
  private static uint P1(uint x) => x ^ BitOperations.RotateLeft(x, 15) ^ BitOperations.RotateLeft(x, 23);
}
