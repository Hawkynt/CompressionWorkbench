...
internal static class PeOverlay {
  ...
  internal static long FindOverlayOffset(Stream stream) {
    try {
      ...
      if (stream.Read(buf[..4]) < 4)
        return -1;
      int peOffset = BinaryPrimitives.ReadInt32LittleEndian(buf[..4]);
      ...
      if (stream.Read(buf[..20]) < 20)
        return -1;
      ushort numSections = BinaryPrimitives.ReadUInt16LittleEndian(buf[2..4]);
      ushort sizeOfOptionalHeader = BinaryPrimitives.ReadUInt16LittleEndian(buf[16..18]);
      ...
      for (int i = 0; i < numSections; i++) {
        if (stream.Read(secBuf) < 40)
          return -1;
        uint rawSize = BinaryPrimitives.ReadUInt32LittleEndian(secBuf.AsSpan(16));
        uint rawPtr = BinaryPrimitives.ReadUInt32LittleEndian(secBuf.AsSpan(20));
  ...
  internal static (FormatDetector.Format Format, long Offset)? ScanForArchive(Stream stream, long startOffset, long maxScan = 1024 * 1024) {
    ...
    long end = Math.Min(stream.Length - 4, startOffset + maxScan);
    stream.Position = startOffset;

    // Read a chunk and scan for signatures
    int chunkSize = (int)Math.Min(end - startOffset + 16, 4 * 1024 * 1024);
    ...
    for (int i = 0; i <= read - 4; i++) {
      ...
      if (i + 6 < read && span[i] == 0x2A && span[i + 1] == 0x2A && span[i + 2] == 0x41 &&
        ...
        long aceOffset = startOffset + i - 7;
      ...
      if (i + 6 < read && span[i] == '-' && span[i + 1] == 'l' &&
        ...
        long lhaOffset = startOffset + i - 2;
  ...
  internal static (FormatDetector.Format Format, long Offset)? DetectEmbeddedArchive(string path) {
    try {
      ...
      long overlayOffset = FindOverlayOffset(fs);
