...
public sealed class AcronisFormatDescriptor : IFormatDescriptor, IArchiveFormatOperations {
  ...
  public string Description => "Acronis True Image classic .tib backup — R/O listing + R/O file extraction via spec-grounded FileMeta chain walk (Listing.MetaOffset → FirstFileMetaRecord(102) → next RecordIndex(108)) with sequential pairing as fallback; FileMeta 102/1/2/5 body shape reverse-engineered as an InputItem attribute stream (uint32 count + N×{uint32 idAndFlags, uint16 size, body}) with the high-value ids decoded — ItemCommon(0x10)=filename+altname, SourceItem(0x40)=path, HardLinkId(0x14), BackupTime(0x50), TimeZone(0x60); per-entry DecodedName surfaced when the 102 body parses; per-blob MD5 integrity check still gates extraction";
