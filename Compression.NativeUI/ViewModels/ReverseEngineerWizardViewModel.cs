...
internal sealed class ReverseEngineerWizardViewModel : ViewModelBase {
  ...
  /// <summary>
  /// Runs an action on the UI thread. The analyzers report progress from worker threads, and only
  /// the view knows which control owns the thread, so it supplies the marshaller.
  /// </summary>
  ...
  public ReverseEngineerWizardViewModel() {
    ...
    BrowseOriginalCommand = new RelayCommand(p => {
      ...
      if (dialog.ShowDialog() == DialogResult.OK) s.OriginalPath = dialog.FileName;
    });
    BrowseArchiveCommand = new RelayCommand(p => {
      ...
      if (dialog.ShowDialog() == DialogResult.OK) s.ArchivePath = dialog.FileName;
    });
  ...
  private async Task RunToolAnalysisAsync(CancellationToken ct) {
    ...
    var report = await reverser.AnalyzeAsync(
      (name, current, total) => {
        this.Marshal(() => {
          this.CurrentProbe = name;
          this.Progress = 100.0 * current / total;
          this.StatusText = $"Probe {current}/{total}: {name}";
        });
      },
      ct
    );

    this.Marshal(() => this.PopulateResults(report));
  }
  ...
  private void RunStaticAnalysis() {
    ...
    var report = StaticFormatAnalyzer.Analyze(samples, (desc, current, total) => {
      this.Marshal(() => {
        this.StatusText = desc;
        this.Progress = 100.0 * current / total;
      });
    });

    this.Marshal(() => this.PopulateStaticResults(report));
  }
