...
internal sealed class CreateOptionsViewModel : INotifyPropertyChanged {
  ...
  public Color MemoryForeground {
    get {
      var (compress, decompress) = EstimateMemoryBytes();
      if (compress <= 0 && decompress <= 0) return Color.Gray;
      if (decompress > AvailableMemory)
        return Color.FromArgb(139, 0, 0); // dark red
      if (compress > AvailableMemory)
        return Color.FromArgb(120, 80, 0); // dark amber
  ...
  public Color MemoryBackground {
    get {
      var (compress, decompress) = EstimateMemoryBytes();
      if (decompress > AvailableMemory)
        return Color.FromArgb(255, 160, 140); // salmon
      if (compress > AvailableMemory)
        return Color.FromArgb(255, 255, 180); // yellow
      return Color.FromArgb(248, 248, 248); // neutral gray
    }
  }
  ...
  private void UpdateMemoryEstimate() {
    ...
    OnPropertyChanged(nameof(MemoryForeground));
