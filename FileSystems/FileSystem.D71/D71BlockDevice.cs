...
/// <summary>
/// Sector-addressable view of the data portion of a standard 70-track D71.
/// The optional per-sector error table in 351062-byte images remains outside
/// the exposed block geometry and is therefore preserved by ordinary writes.
/// </summary>
public sealed class D71BlockDevice : IRandomAccessBlockDevice {
  public const int LogicalSectorSize = 256;
  public const int SectorCount = 1366;
  ...
  public D71BlockDevice(Stream stream, bool writable, bool leaveOpen = true) {
    ArgumentNullException.ThrowIfNull(stream);
    if (!stream.CanRead || !stream.CanSeek)
      throw new ArgumentException("D71 block access requires a readable, seekable stream.", nameof(stream));
    if (stream.Length < DataLength)
      throw new InvalidDataException($"D71 image is {stream.Length} bytes; at least {DataLength} bytes are required.");
    if (writable && !stream.CanWrite)
      throw new ArgumentException("Writable D71 block access requires a writable stream.", nameof(stream));
    _stream = stream;
  ...
  public void Trim(long firstBlock, long blockCount) {
    ...
    throw new NotSupportedException("D71 has no discard/trim primitive; filesystem allocation owns free-sector contents.");
  }
  ...
  private void EnsureWritable() {
    if (!CanWrite) throw new NotSupportedException("The D71 block device was opened read-only.");
  }
