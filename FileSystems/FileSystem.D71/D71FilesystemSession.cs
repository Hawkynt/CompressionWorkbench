...
/// <summary>
/// Root-only CBM DOS namespace session over a 1571 D71 sector device. Node ids
...
public sealed class D71FilesystemSession : IFilesystemSession {
  ...
  public D71FilesystemSession(
    ...
    if (device.Geometry.LogicalBlockSize != SectorSize || device.Geometry.BlockCount < D71BlockDevice.SectorCount)
      throw new InvalidDataException("1571 CBM DOS requires a 1366-sector, 256-byte D71 block device.");
    if (!readOnly && !device.CanWrite)
      throw new ArgumentException("Writable D71 session requires a writable block device.", nameof(device));
    if (!readOnly && !profile.CanMountWritable)
      throw new NotSupportedException("This exact D71 media profile is not safe for writable mounting.");
  ...
  public FilesystemNodeInfo Stat(FilesystemNodeId nodeId) {
    lock (_gate) {
      ...
      if (!_states.TryGetValue(nodeId, out var state))
        throw new FileNotFoundException($"D71 node {nodeId.Value}:{nodeId.Generation} is unknown.");
      if (!state.Linked)
  ...
  public IFilesystemFileHandle OpenFile(FilesystemNodeId nodeId, FileAccess access) {
    lock (_gate) {
      ThrowIfDisposed();
      if (nodeId == RootNodeId) throw new UnauthorizedAccessException("The D71 root directory is not a regular file.");
      if (access != FileAccess.Read && _readOnly)
        throw new NotSupportedException("The D71 session was opened read-only.");
  ...
  public FilesystemNodeId CreateFile(FilesystemNodeId parentDirectory, string name) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => D71Modifier.AddFile(stream, name, [], 0x82));
  ...
  public FilesystemNodeId CreateDirectory(FilesystemNodeId parentDirectory, string name)
    => throw new NotSupportedException("1571 CBM DOS has a flat root directory and cannot create subdirectories.");
  ...
  public void DeleteFile(FilesystemNodeId parentDirectory, string name) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => {
        if (!D71Modifier.RemoveFile(stream, state.Name, wipeData: true))
  ...
  public void RemoveDirectory(FilesystemNodeId parentDirectory, string name)
    => throw new NotSupportedException("1571 CBM DOS has no subdirectories.");
  ...
  public void Rename(
    ...
    lock (_gate) {
      ...
      if (_idsByName.TryGetValue(newName, out var targetId) && targetId != sourceId) {
        ...
        ApplyStreamMutation(stream => {
          if (!D71Modifier.RemoveFile(stream, target.Name, wipeData: true))
  ...
  public void CreateHardLink(FilesystemNodeId existingNode, FilesystemNodeId newParent, string newName)
    => throw new NotSupportedException("1571 CBM DOS has no hard links.");

  public FilesystemNodeId CreateSymbolicLink(FilesystemNodeId parentDirectory, string name, string target)
    => throw new NotSupportedException("1571 CBM DOS has no symbolic links.");
  ...
  public void SetMetadata(FilesystemNodeId nodeId, FilesystemMetadataPatch patch)
    => throw new NotSupportedException("The generic metadata patch has no lossless 1571 CBM DOS mapping.");
  ...
  public IFilesystemTransaction BeginTransaction()
    => throw new NotSupportedException("1571 CBM DOS has no crash-atomic journal/transaction primitive.");
  ...
  private static byte[] ReadWholeImage(IRandomAccessBlockDevice device) {
    ...
    if (blocks != D71BlockDevice.SectorCount)
      throw new EndOfStreamException($"D71 block device returned {blocks} sectors; expected {D71BlockDevice.SectorCount}.");
    return data;
  }
  ...
  private void IndexInitialNamespace() {
    foreach (var record in ScanDirectory(_image)) {
      if (_idsByName.ContainsKey(record.Name))
        throw new InvalidDataException($"D71 directory contains duplicate name '{record.Name}'; mount would be ambiguous.");
      var nodeId = new FilesystemNodeId(_nextNodeId++, 1);
  ...
  private void FlushState(NodeState state) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => {
        if (!D71Modifier.RemoveFile(stream, oldName, wipeData: false))
          throw new IOException($"CBM DOS entry '{oldName}' disappeared while flushing an open handle.");
        D71Modifier.AddFile(stream, oldName, state.Data, state.FileType);
      });
  ...
  private void CommitChangedBlocks(byte[] candidate) {
    if (candidate.Length != D71BlockDevice.DataLength)
      throw new InvalidDataException("CBM DOS mutation changed the fixed D71 data geometry.");
    for (var block = 0; block < D71BlockDevice.SectorCount; ++block) {
  ...
  private NodeState RequireLinkedState(FilesystemNodeId nodeId) {
    if (!_states.TryGetValue(nodeId, out var state) || !state.Linked)
      throw new FileNotFoundException($"D71 node {nodeId.Value}:{nodeId.Generation} is not linked in this session.");
    return state;
  }
  ...
  private static IReadOnlyList<DirectoryRecord> ScanDirectory(byte[] image) {
    ...
    while (track != 0) {
      if (!visited.Add((track, sector))) throw new InvalidDataException("D71 directory chain contains a loop.");
      var offset = SectorOffset(track, sector);
      if (offset < 0 || offset + SectorSize > image.Length)
        throw new InvalidDataException("D71 directory points outside the 1571 geometry.");
  ...
  private static List<(int Track, int Sector)> WalkChain(byte[] image, int track, int sector) {
    ...
    var visited = new HashSet<(int, int)>();
    while (track != 0) {
      if (!visited.Add((track, sector))) throw new InvalidDataException("D71 file sector chain contains a loop.");
      var offset = SectorOffset(track, sector);
      if (offset < 0 || offset + SectorSize > image.Length)
        throw new InvalidDataException("D71 file sector chain points outside the 1571 geometry.");
  ...
  private void RequireRoot(FilesystemNodeId nodeId) {
    if (nodeId != RootNodeId) throw new DirectoryNotFoundException("1571 CBM DOS has only the root directory.");
  }

  private void EnsureWritable() {
    if (_readOnly) throw new NotSupportedException("The D71 session was opened read-only.");
  }
  ...
  private sealed class FileHandle(D71FilesystemSession session, NodeState state, FileAccess access) : IFilesystemFileHandle {
    private bool _disposed;
    ...
    public void Write(long offset, ReadOnlySpan<byte> source) {
      lock (session._gate) {
        ...
        if (end > MaximumFileBytes) throw new IOException("CBM DOS file exceeds the maximum D71 data capacity.");
        var data = state.Data ?? [];
