...
public sealed class UdfBlockMover : IFilesystemBlockMover {
  ...
  public void MoveExtent(Stream image, long srcOffset, long dstOffset, long length, bool zeroSource = false) {
    ...
    try {
      ...
      // reaches it. Without this barrier the OS could reorder writes such
      // that the updated FE allocation descriptor referencing dst commits
      // BEFORE the dst bytes themselves — crash-window where the file points
      // at garbage.
      image.Flush();
