...
/// </remarks>
public sealed class Nilfs1BlockMover : IFilesystemBlockMover {

  private Nilfs1Layout.Layout? _layout;
  ...
  public void Init(Stream image) {
    ...
    this._layout = Nilfs1Layout.Read(image);
    if (this._layout == null)
      throw new InvalidDataException("Nilfs1: the volume is not one this reads.");
  ...
  public void UpdateAllocationAfterMove(Stream image, string fileName, long oldOffset, long newOffset, long length) {
    ...
    if (newOffset < this._layout!.PayloadStart)
      throw new NotSupportedException(
        "Nilfs1: a payload cannot start before its segment's own payloads do — the format writes " +
        "the position as an offset from there, and it has no way to say a negative one.");

    if (newOffset + length > this._layout.PayloadEnd)
      throw new NotSupportedException(
        "Nilfs1: a payload cannot reach past the next segment's header, which the reader finds by " +
        "carrying on from where these payloads end.");

    if (!this._offsetField.Remove(oldOffset, out var field))
      throw new InvalidOperationException(
        $"Nilfs1: the directory names no payload at {oldOffset}, so '{fileName}' cannot be repointed.");
