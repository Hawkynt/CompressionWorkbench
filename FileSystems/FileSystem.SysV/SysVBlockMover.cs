...
/// </remarks>
public sealed class SysVBlockMover : IFilesystemBlockMover {
...
public void UpdateAllocationAfterMove(Stream image, string fileName, long oldOffset, long newOffset, long length) {
    ...
    if (newOffset % this._blockSize != 0)
      throw new NotSupportedException(
        $"SysV: {newOffset} is not on a {this._blockSize}-byte block boundary, which is all a " +
    ...
    if (newBlock > 0xFFFFFF)
      throw new NotSupportedException(
        $"SysV: block {newBlock} is past the 24 bits an inode's pointer holds.");
    ...
    if (pointerOffset < 0)
      throw new InvalidOperationException(
        $"SysV: no pointer run names block {oldBlock}, so '{fileName}' cannot be repointed.");

    // A pointer inside an inode is three bytes, low byte first; one inside an
    // indirect block is a plain four-byte word.
    var stride = inInode ? SysVReader.InodePointerBytes : SysVReader.IndirectPointerBytes;
    Span<byte> pointer = stackalloc byte[SysVReader.IndirectPointerBytes];
