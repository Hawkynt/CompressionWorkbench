...
public sealed class MinixV2FormatDescriptor : IFormatDescriptor, IArchiveFormatOperations, IArchiveCreatable, IArchiveShrinkable, IArchiveModifiable, IArchiveDefragmentable, IFormatOptionsSchema, ILayoutOptimizable, IFilesystemExtentMap, IWipeEmpty {
  ...
  /// <summary>
  /// Adds (or replaces by name) files inside an existing Minix v2 image via
  /// <see cref="MinixV2InPlaceModifier"/> — TRUE in-place O(touched bytes) I/O
  ...
  public void Add(Stream archive, IReadOnlyList<ArchiveInputInfo> inputs) {
    try {
      foreach (var (name, data) in FilesOnly(inputs)) {
        MinixV2InPlaceModifier.RemoveFile(archive, name, wipeData: true);
        MinixV2InPlaceModifier.AddFile(archive, name, data);
      }
    } catch (IOException) {
      archive.Position = 0;
  ...
  /// <summary>Removes the named entries in-place via <see cref="MinixV2InPlaceModifier"/>.</summary>
  public void Remove(Stream archive, string[] entryNames) {
    var leftover = new List<string>();
    foreach (var name in entryNames) {
      var leaf = name.Replace('\\', '/').TrimStart('/');
      if (leaf.Contains('/') || !MinixV2InPlaceModifier.RemoveFile(archive, leaf, wipeData: true))
        leftover.Add(name);
    }
  ...
  private byte[] BuildImage(IReadOnlyList<(string Name, byte[] Data)> files) {
    ...
    using var w = new MinixV2Writer(ms, leaveOpen: true);
  ...
  /// <see cref="MinixV2ExtentMap" /> now does.
  /// </remarks>
  ...
  private void DefragmentWithPlanner(Stream archive, DefragOptions options) {
    archive.Position = 0;
    var mover = new MinixV2BlockMover();
    mover.Init(archive);

    var extents = MinixV2ExtentMap.Enumerate(archive).ToList();
    ...
    archive.Position = 0;
    var postExtents = MinixV2ExtentMap.Enumerate(archive).ToList();
    options.OnProgress?.Invoke(new DefragProgressEvent(
  ...
  private static List<(string Name, byte[] Data)> ReadEntries(Stream stream) {
    ...
    using var reader = new MinixV2Reader(stream);
  ...
  public void RebuildStreaming(Stream source, Stream target, LayoutRebuildOptions options) {
    ...
    using (var reader = new MinixV2Reader(source)) {
    ...
    using var writer = new MinixV2Writer(target, leaveOpen: true) {
  ...
  public long WipeUnusedSpace(Stream image, bool wipeClusterTips = true, bool wipeDeletedEntries = true) {
    ...
    var extents = MinixV2ExtentMap.Enumerate(image).ToList();
