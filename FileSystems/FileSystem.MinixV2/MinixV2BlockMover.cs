...
/// </summary>
/// <remarks>
/// <para>A Minix file's bytes are addressed one zone at a time by four-byte
...
/// </remarks>
public sealed class MinixV2BlockMover : IFilesystemBlockMover {
  ...
  public void Init(Stream image) {
    ...
    this._firstDataZone = this._firstDataByte / MinixV2Reader.ZoneSize;
    this._volumeZones = image.Length / MinixV2Reader.ZoneSize;
  }

  /// <summary>A zone, which is a block at the zone size this writer uses.</summary>
  public int BlockSize => MinixV2Reader.ZoneSize;
...
public void UpdateAllocationAfterMove(Stream image, string fileName, long oldOffset, long newOffset,
    ...
    if (newZone > uint.MaxValue)
      throw new NotSupportedException(
        $"Minix: zone {newZone} is past what a thirty-two-bit pointer holds.");
    ...
    for (var i = 0; i < zones; ++i) {
      BinaryPrimitives.WriteUInt32LittleEndian(pointer, (uint)(newZone + i));
  ...
  private long FindPointerNaming(Stream image, string fileName, long offset, int zones) {
    ...
    foreach (var entry in candidates) {
      foreach (var (runOffset, runLength, pointerOffset) in reader.EnumerateDataExtents(entry)) {
        ...
        if (runLength < (long)zones * MinixV2Reader.ZoneSize) continue;
