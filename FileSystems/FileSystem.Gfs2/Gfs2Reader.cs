...
///   records live inline in the inode block (stuffed directories).</description></item>
...
public sealed class Gfs2Reader : IDisposable {
  ...
  private void Parse() {
    ...
    // Walk the root inode and every nested stuffed directory. Errors are
  ...
  /// <summary>
  /// Walks one stuffed directory and descends into native child directories.
  /// ExHash directories are deliberately skipped. A global visited set and a
  /// conservative depth cap prevent malformed directory cycles from recursing
  /// indefinitely.
  /// </summary>
  private void WalkDirectory(ulong inodeBlock, string prefix, HashSet<ulong> visited, int depth) {
    ...
    var directoryBlock = this.Block((long)inodeBlock).AsSpan();
    ...
    // Single-leaf inline directory only (di_height == 0). ExHash directory
    // leaves/hashes remain out of scope.
    if (diHeight != 0)
      return;
    ...
    var dentries = directoryBlock[DinodeHeaderSize..];
    this.ParseDentries(
      dentries,
      diMtime,
  ...
  private void ParseDentries(ReadOnlySpan<byte> area, ulong dirMtimeBe, uint maxEntries,
    ...
    while (off + direntHeaderSize <= area.Length && count < maxEntries) {
      ...
      // di_entries counts all live directory records, including "." and "..".
      count++;
      ...
      // Skip "." and ".." but count them toward di_entries above.
      if (name is "." or "..") {
        off += recLen;
        continue;
      }
