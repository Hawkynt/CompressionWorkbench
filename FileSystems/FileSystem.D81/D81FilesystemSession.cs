...
/// <summary>
/// Root-only CBM DOS namespace session over a 1581 D81 sector device. Node ids
/// remain stable for the lifetime of the session, including across rename and
/// unlink while an already-open handle still exists.
/// </summary>
public sealed class D81FilesystemSession : IFilesystemSession {
  ...
  private const int MaximumFileBytes = 3160 * 254;
  ...
  public D81FilesystemSession(
    ...
    if (device.Geometry.LogicalBlockSize != SectorSize || device.Geometry.BlockCount < D81BlockDevice.SectorCount)
      throw new InvalidDataException("1581 CBM DOS requires a 3200-sector, 256-byte D81 block device.");
    if (!readOnly && !device.CanWrite)
      throw new ArgumentException("Writable D81 session requires a writable block device.", nameof(device));
    if (!readOnly && !profile.CanMountWritable)
      throw new NotSupportedException("This exact D81 media profile is not safe for writable mounting.");
  ...
  public FilesystemNodeInfo Stat(FilesystemNodeId nodeId) {
    lock (_gate) {
      ...
      if (!_states.TryGetValue(nodeId, out var state))
        throw new FileNotFoundException($"D81 node {nodeId.Value}:{nodeId.Generation} is unknown.");
      if (!state.Linked)
  ...
  public IFilesystemFileHandle OpenFile(FilesystemNodeId nodeId, FileAccess access) {
    lock (_gate) {
      ThrowIfDisposed();
      if (nodeId == RootNodeId) throw new UnauthorizedAccessException("The D81 root directory is not a regular file.");
      if (access != FileAccess.Read && _readOnly)
        throw new NotSupportedException("The D81 session was opened read-only.");
  ...
  public FilesystemNodeId CreateFile(FilesystemNodeId parentDirectory, string name) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => D81Modifier.AddFile(stream, name, [], 0x82));
  ...
  public FilesystemNodeId CreateDirectory(FilesystemNodeId parentDirectory, string name)
    => throw new NotSupportedException("1581 native partitions/subdirectories are not implemented by this flat CBM DOS session.");
  ...
  public void DeleteFile(FilesystemNodeId parentDirectory, string name) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => {
        if (!D81Modifier.RemoveFile(stream, state.Name, wipeData: true))
  ...
  public void Rename(
    ...
    lock (_gate) {
      ...
      if (_idsByName.TryGetValue(newName, out var targetId) && targetId != sourceId) {
        ...
        ApplyStreamMutation(stream => {
          if (!D81Modifier.RemoveFile(stream, target.Name, wipeData: true))
  ...
  public void CreateHardLink(FilesystemNodeId existingNode, FilesystemNodeId newParent, string newName)
    => throw new NotSupportedException("1581 CBM DOS has no hard links.");

  public FilesystemNodeId CreateSymbolicLink(FilesystemNodeId parentDirectory, string name, string target)
    => throw new NotSupportedException("1581 CBM DOS has no symbolic links.");
  ...
  public void SetMetadata(FilesystemNodeId nodeId, FilesystemMetadataPatch patch)
    => throw new NotSupportedException("The generic metadata patch has no lossless 1581 CBM DOS mapping.");
  ...
  public IFilesystemTransaction BeginTransaction()
    => throw new NotSupportedException("1581 CBM DOS has no crash-atomic journal/transaction primitive.");
  ...
  private static byte[] ReadWholeImage(IRandomAccessBlockDevice device) {
    ...
    if (blocks != D81BlockDevice.SectorCount)
      throw new EndOfStreamException($"D81 block device returned {blocks} sectors; expected {D81BlockDevice.SectorCount}.");
    return data;
  }
  ...
  private void IndexInitialNamespace() {
    foreach (var record in ScanDirectory(_image)) {
      if (_idsByName.ContainsKey(record.Name))
        throw new InvalidDataException($"D81 directory contains duplicate name '{record.Name}'; mount would be ambiguous.");
      var nodeId = new FilesystemNodeId(_nextNodeId++, 1);
  ...
  private void FlushState(NodeState state) {
    lock (_gate) {
      ...
      ApplyStreamMutation(stream => {
        if (!D81Modifier.RemoveFile(stream, oldName, wipeData: false))
          throw new IOException($"CBM DOS entry '{oldName}' disappeared while flushing an open handle.");
        D81Modifier.AddFile(stream, oldName, state.Data, state.FileType);
      });
  ...
  private void CommitChangedBlocks(byte[] candidate) {
    if (candidate.Length != D81BlockDevice.DataLength)
      throw new InvalidDataException("CBM DOS mutation changed the fixed D81 data geometry.");
    for (var block = 0; block < D81BlockDevice.SectorCount; ++block) {
  ...
  private NodeState RequireLinkedState(FilesystemNodeId nodeId) {
    if (!_states.TryGetValue(nodeId, out var state) || !state.Linked)
      throw new FileNotFoundException($"D81 node {nodeId.Value}:{nodeId.Generation} is not linked in this session.");
    return state;
  }
  ...
  private static IReadOnlyList<DirectoryRecord> ScanDirectory(byte[] image) {
    ...
    while (track != 0) {
      if (!visited.Add((track, sector))) throw new InvalidDataException("D81 directory chain contains a loop.");
      var offset = SectorOffset(track, sector);
      if (offset < 0 || offset + SectorSize > image.Length)
        throw new InvalidDataException("D81 directory points outside the 1581 geometry.");
  ...
  private static List<(int Track, int Sector)> WalkChain(byte[] image, int track, int sector) {
    ...
    var visited = new HashSet<(int, int)>();
    while (track != 0) {
      if (!visited.Add((track, sector))) throw new InvalidDataException("D81 file sector chain contains a loop.");
      var offset = SectorOffset(track, sector);
      if (offset < 0 || offset + SectorSize > image.Length)
        throw new InvalidDataException("D81 file sector chain points outside the 1581 geometry.");
  ...
  private static int SectorOffset(int track, int sector) {
    if (track is < 1 or > TotalTracks || sector is < 0 or >= SectorsPerTrack) return -1;
  ...
  private void RequireRoot(FilesystemNodeId nodeId) {
    if (nodeId != RootNodeId) throw new DirectoryNotFoundException("The native D81 driver currently exposes only the root directory.");
  }

  private void EnsureWritable() {
    if (_readOnly) throw new NotSupportedException("The D81 session was opened read-only.");
  }
  ...
  private sealed class FileHandle(D81FilesystemSession session, NodeState state, FileAccess access) : IFilesystemFileHandle {
    private bool _disposed;
    ...
    public void Write(long offset, ReadOnlySpan<byte> source) {
      lock (session._gate) {
        ...
        if (end > MaximumFileBytes) throw new IOException("CBM DOS file exceeds the maximum D81 data capacity.");
        var data = state.Data ?? [];
