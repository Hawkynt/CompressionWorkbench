...
public sealed class MinixV1Reader : IDisposable {
  ...
  /// <summary>Byte offset where the inode table starts.</summary>
  public long InodeTableStart => this.InodeTableOffset();

  /// <summary>First zone that may hold file data: the one past the inode table.</summary>
  public long FirstDataZoneOffset {
    get {
      var inodeCount = BinaryPrimitives.ReadUInt16LittleEndian(_data.AsSpan(SuperblockOffset, 2));
  ...
  private byte[] ReadZones(uint[] zones, uint size) {
    ...
    if (remaining > 0) {
      ...
      else AppendHole(ms, ref remaining, (long)(BlockSize / 2) * BlockSize);
    }

    // double indirect
    if (remaining > 0) {
      if (zones[8] != 0) ReadIndirect(ms, zones[8], ref remaining, 2);
      else AppendHole(ms, ref remaining, (long)(BlockSize / 2) * BlockSize * (BlockSize / 2));
    }
  ...
  /// </remarks>
  public IEnumerable<(long Offset, long Length, long PointerOffset)> EnumerateDataExtents(MinixV1Entry entry) {
    ...
    foreach (var (zone, pointerOffset) in this.EnumerateZonePointers(zones, inodeOffset)) {
      ...
      if (runPointer >= 0 && zone == runFirstZone + runZones
          && pointerOffset == runPointer + (long)runZones * 2) {
        ++runZones;
        continue;
      }
  ...
  private IEnumerable<(uint Zone, long PointerOffset)> EnumerateZonePointers(uint[] zones, long inodeOffset) {
    for (var i = 0; i < 7; ++i)
      yield return (zones[i], inodeOffset + 14 + (long)i * 2);
  ...
  private IEnumerable<(uint Zone, long PointerOffset)> EnumerateIndirectPointers(uint zone, int levels) {
    var reach = 1L;
    for (var i = 0; i < levels; ++i) reach *= BlockSize / 2;
    ...
    for (var i = 0; i < BlockSize / 2; ++i) {
      var at = blockOffset + (long)i * 2;
      var pointer = BinaryPrimitives.ReadUInt16LittleEndian(_data.AsSpan((int)at, 2));
      if (levels <= 1) {
