...
/// </summary>
/// <remarks>
/// <para>An X68000 volume is laid out like FAT — reserved sectors, the file
/// allocation tables, a fixed root directory, then the data area — but it puts
/// several BPB fields at its own offsets, which is why the FAT walker does not
/// recognise it. The structures themselves are the same: a 32-byte directory
/// entry carrying a first cluster, and a chain in the table.</para>
///
...
/// instead.</para>
/// </remarks>
public sealed class Human68kBlockMover : IFilesystemBlockMover {
  ...
  public void Init(Stream image) {
    ...
    this._bytesPerSector = BinaryPrimitives.ReadUInt16LittleEndian(boot[0x0B..]);
    if (this._bytesPerSector is < 256 or > 4096) this._bytesPerSector = 512;
    this._sectorsPerCluster = boot[0x0D] == 0 ? 1 : boot[0x0D];
    this._reservedSectors = BinaryPrimitives.ReadUInt16LittleEndian(boot[0x0E..]);
    this._fatCount = boot[0x14];
    this._rootEntries = BinaryPrimitives.ReadUInt16LittleEndian(boot[0x15..]);
    this._sectorsPerFat = BinaryPrimitives.ReadUInt16LittleEndian(boot[0x1A..]);
    if (this._fatCount is < 1 or > 4)
      throw new InvalidDataException("Human68k: the boot sector does not describe a FAT layout.");

    this._clusterSize = this._sectorsPerCluster * this._bytesPerSector;
    this._fatBase = (long)this._reservedSectors * this._bytesPerSector;
...
public void UpdateAllocationAfterMove(Stream image, string fileName, long oldOffset, long newOffset, long length) {
    ...
    if (!this.PatchDirectoryEntry(image, oldFirst, newFirst))
      throw new InvalidOperationException(
        $"Human68k: no directory entry starts at cluster {oldFirst}, so '{fileName}' cannot be repointed.");
    image.Flush();
