...
public sealed class XfsReader : IDisposable {
  ...
  private void Parse() {
    ...
    _inodeSize = BinaryPrimitives.ReadUInt16BigEndian(_data.AsSpan(84));
    _rootIno = BinaryPrimitives.ReadUInt64BigEndian(_data.AsSpan(56));
    _agCount = BinaryPrimitives.ReadUInt32BigEndian(_data.AsSpan(80));
    _agBlocks = BinaryPrimitives.ReadUInt32BigEndian(_data.AsSpan(102));
    _agBlkLog = _data[88];
  ...
  private void ReadDirectory(ulong ino, string basePath) {
    ...
    var size = (long)BinaryPrimitives.ReadUInt64BigEndian(_data.AsSpan(ioff + 8));

    if (format == 1) {
      // Short-form directory: inline data after inode core
      ReadShortFormDir(ioff + 176, Math.Min((int)size, (int)_inodeSize - 176), basePath);
    } else if (format == 2) {
  ...
  private void ReadShortFormDir(int dataOff, int dataLen, string basePath) {
    ...
    for (int i = 0; i < count + i8count && pos + 3 < dataOff + dataLen; i++) {
      ...
      bool isDir = false;
      long childSize = 0;
      if (childOff >= 0 && childOff + 16 <= _data.Length) {
        var childMode = BinaryPrimitives.ReadUInt16BigEndian(_data.AsSpan((int)childOff + 2));
        isDir = (childMode & 0xF000) == 0x4000;
        childSize = (long)BinaryPrimitives.ReadUInt64BigEndian(_data.AsSpan((int)childOff + 8));
      }
  ...
  public byte[] Extract(XfsEntry entry) {
    ...
    if (format == 1) {
      // Local/inline data
      var dataOff = ioff + 176;
      var len = (int)Math.Min(size, _inodeSize - 176);
