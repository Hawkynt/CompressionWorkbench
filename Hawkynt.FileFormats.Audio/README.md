...
**Oracle** names what the row has been held against that was not written here: a published
specification, the format's own reference tool, or libavcodec through `ffmpeg`. Encoding with our
encoder and decoding with our decoder proves only that the two halves agree with each other —
WavPack, TTA, ALAC and CRI ADX each round-tripped perfectly for months while exchanging a file
with nothing else. A dash means no such check exists and the row rests on a round trip alone,
which is the single most useful thing this column has to say. Authority runs in that order too:
where our G.726 and ffmpeg disagree, the ITU-T reference decides, and it decided against ffmpeg.
...
| Id | Family | State | Historical identifiers | Oracle | Notes |
| --- | --- | :---: | --- | --- | --- |
| `Codec.Pcm` | PCM | R/W | `0x0001`, `0x0003` | [ffmpeg][ffmpeg] probes and decodes our PCM in six containers | Raw integer PCM (8/16/24/32-bit), interleaved + planar, channel splitting |
...
| `Codec.ImaAdpcm` | ADPCM | R/W | `0x0011` | [ffmpeg][ffmpeg] — QuickTime `ima4` frames in AIFC | IMA / Intel ADPCM + QuickTime `ima4` packet variant. `0x0039` is Roland RDAC in the IANA registry, not an IMA alias |
| `Codec.MsAdpcm` | ADPCM | R/W | `0x0002` | — | Microsoft ADPCM encode + decode |
| `Codec.Gsm610` | Speech | R/W | `0x0031`, `0x0086`, `0x00A1`, `0x0155` | [RPE-LTP reference][gsm] vectors, bit-exact both ways | GSM 06.10 RPE-LTP, bit-exact with the ETSI reference in both directions. `0x0031` uses real Microsoft WAV49 65-byte blocks; the vendor aliases are identifier-only |
| `Codec.Mp3` | Lossy | R/W ⚠️ | `0x0050`, `0x0055`, `0x0700` | [ffmpeg][ffmpeg] on [LAME][lame] streams; Layer III within 2 counts | Decoder covers Layers I/II/III; `0x0055` MPEGLAYER3WAVEFORMAT is validated WAVE R/W with the checked-in Layer III encoder. `0x0050` remains WAVE decode-only; `0x0700` is identifier-only |
| `Codec.Aac` | Lossy | R/W ⚠️ | `0x00B0`, `0x00FF`, `0x0180`, `0x0AAC`, `0x1600`, `0x4143`, `0x706D`, `0xA106`, `0x2006`, `0x2007` | [ffmpeg][ffmpeg] ADTS and MP4 streams | AAC-LC encode and decode pinned against ffmpeg's own stream, with advanced-profile limits; WAVE `0x00FF` RAW_AAC1 and `0x1600` ADTS are validated R/W, while the vendor registrations remain identifier-only |
...
| `Codec.Opus` | Lossy | R/W ⚠️ | — | [libopus][opus] stream, trimmed to the exact granule | Opus path with documented CELT/SILK/hybrid limits |
| `Codec.Flac` | Lossless | R/W | `0xF1AC` | [flac][flac] verifies and decodes ours; [ffmpeg][ffmpeg] on FLAC-in-Ogg | FLAC frame-level encode + decode; the WAVE tag is not routed; native `.flac` is |
...
| `Codec.G72x` | ADPCM | R/W | `0x0040`, `0x0045`, `0x0064`, `0x0085`, `0x008B`, `0x0140`, `0x4243`, `0xA105`, `0xA107` | [ITU-T G.191][g191] known answers — ffmpeg differs and is the one that is wrong | ITU-T G.726 16/24/32/40 kbit encode + decode, bit-exact with the ITU-T G.191 reference. `0x0040`, `0x0045` and `0x0064` are validated on the WAVE route; the vendor aliases are profile/framing work |
| `Codec.Tta` | Lossless | R/W | — | [ffmpeg][ffmpeg] stream, byte-exact | True Audio TTA1 encode + decode |
| `Codec.Shorten` | Lossless | R/W | — | — | Shorten v2 decode including best-effort QLPC + DIFF0-3 encoder |
| `Codec.Alac` | Lossless | R/W | — | [ffmpeg][ffmpeg] frames, byte-exact (decode side only) | Apple Lossless 16/24-bit paths |
| `Codec.WavPack` | Lossless | R/W | `0x5756` | [wvunpack][wavpack] verifies ours; [ffmpeg][ffmpeg] stream byte-exact | WavPack v4/v5 lossless including IEEE-float handling; the WAVE tag is not routed; native `.wv` is |
| `Codec.CriAdx` | ADPCM | R/W | — | [ffmpeg][ffmpeg] stream, byte-exact | CRI ADX high-pass-derived coefficients |
| `Codec.Brr` | ADPCM | R/W | — | [BRRtools][brrtools] 3.15 decode vectors | SNES S-DSP BRR filters 0-3 and hardware-style wrap |
...
| `Codec.AicaAdpcm` | ADPCM | R/W | `0x0020` | [ffmpeg][ffmpeg] WAVE `0x0020` stream, byte-exact | Yamaha AICA / ADPCM-B (Dreamcast); the WAVE Yamaha ADPCM route decodes bit-exactly against libavcodec, mono and stereo |
| `Codec.G722` | Speech | R/W | `0x0065`, `0x028F` | [ffmpeg][ffmpeg] known-answer frame | ITU-T G.722 sub-band ADPCM @ 64 kbit |
...
| `Codec.Ra144` | Speech | R | `0x2002` | [ffmpeg][ffmpeg] stream, byte-exact, plus its peak and RMS | RealAudio 14.4 / lpcJ |
...
| `Codec.Nellymoser` | Lossy | R | — | [ffmpeg][ffmpeg] stream, RMS within 4 counts | Nellymoser / Flash |
...
| `Codec.Ac3` | Lossy | R/W ⚠️ | `0x0092`, `0x0241`, `0x2000` | [ffmpeg][ffmpeg] streams sample-by-sample; [A/52][a52] for the exponent rules | AC-3 / E-AC-3 encode + decode with advanced-feature boundaries; S/PDIF-encapsulated variants are not routed |
| `Codec.Wma` | Lossy | R ⚠️ | `0x0160`, `0x0161` | [ffmpeg][ffmpeg] ASF streams, peak error 1 count | WMA v1/v2; ASF carriage is read through `FileFormat.Asf`. Measured against libavcodec the decode is the right tone at the wrong shape, so the WAVE tags are recognised but deliberately not routed |
...
| `Codec.Dts` | Lossy | R/W | `0x0008`, `0x0190`, `0x2001` | [ffmpeg][ffmpeg] streams, within 1 LSB | DTS Coherent Acoustics core encode + decode; wrapper tags are not routed |
...
| `Codec.AmrNb` | Speech | R/W | `0x0136`, `0x4201`, `0x7A21`, `0x7A22` | [ffmpeg][ffmpeg] frame — the pinned samples are our own decode | 3GPP AMR narrowband, all 8 modes; the WAVE aliases are not routed; native `.amr` is |
| `Codec.AmrWb` | Speech | R/W | `0xA104` | [ffmpeg][ffmpeg] frame — the pinned samples are our own decode | 3GPP AMR wideband / G.722.2, all 9 modes; the WAVE alias is not routed; native `.awb` is |
...
| `Codec.AdpcmX` | ADPCM | R | `0x5346` | [ffmpeg][ffmpeg] on the SWF variant only | IMA DK3/DK4/EACS/SEAD, EA R1-R3, THP/AFC, SWF, 4XM, Xan, Interplay, SDX2, DERF, Gremlin; the SWF WAVE route decodes per block, bit-exactly against libavcodec |
...
| `Codec.Sbc` | Lossy | R | — | [ffmpeg][ffmpeg] stream, byte-exact | Bluetooth SBC + mSBC |
...
| `Codec.Qoa` | Lossy (DPCM) | R/W | — | [ffmpeg][ffmpeg] stream, byte-exact | Quite OK Audio sign-LMS slices |
| `Codec.Dfpwm` | Lossy (1-bit) | R/W | `0xC13A` | [ffmpeg][ffmpeg] WAVE `0xC13A` stream, byte-exact | DFPWM1a encode + decode; carried as WAVE_FORMAT_EXTENSIBLE and decoded bit-exactly against libavcodec |
...
| Id | Format | State | Decode | Encode | Demux | Mux | Oracle | Notes |
| --- | --- | :---: | :---: | :---: | :---: | :---: | --- | --- |
| `Aac` | AAC (ADTS) | WORM | ✅ | ✅ | ✅ | ✅ | [ffmpeg][ffmpeg] ADTS stream | AAC ADTS route; ADTS remux in both directions; codec profile limits apply |
...
| `Aea` | Sony ATRAC1 / MiniDisc (.aea) | WORM | ✅ | ✅ | ✅ | ✅ | — | Sony MiniDisc AEA / ATRAC1; fixed-rate 212-byte sound units, mono/stereo encode at 44100 Hz, packet-preserving remux |
...
| `Akb` | Square Enix AKB / AKB2 | WORM | ✅ | ✅ | ✅ | ✅ | [vgmstream](https://github.com/vgmstream/vgmstream) + [Memoria](https://github.com/Albeoris/Memoria) | Classic v0/v2/v3 and AKB2; PCM16LE, MS-ADPCM, Ogg Vorbis, classic M4A/AAC and classic-v3 Ogg XOR; byte-preserving remux |
...
| `Ast` | AST (GameCube/Wii stream) | WORM | ✅ | ✅ | ✅ | ✅ | — | GameCube/Wii AST; PCM16 and AFC blocks decode and encode, and the BLCK layout survives a demux/mux round trip |
...
| `Caf` | CAF (Core Audio Format) | WORM | ✅ | ✅ | ✅ | ✅ | [ffmpeg][ffmpeg] probes and decodes ours | Apple Core Audio Format, LPCM `desc`/`data` chunks |
...
| `Dfpwm` | DFPWM1a (ComputerCraft) | WORM | — | — | — | — | [ffmpeg][ffmpeg], through the WAVE `0xC13A` route | Headerless DFPWM1a convention |
...
| `Dts` | DTS (Coherent Acoustics) | R | ✅ | ✅ | — | — | [ffmpeg][ffmpeg] streams, within 1 LSB | Raw DTS core; HD extensions are not silently treated as core support |
...
| `Flac` | FLAC | WORM | ✅ | ✅ | — | — | [flac][flac] verifies and decodes ours; [ffmpeg][ffmpeg] agrees | FLAC stream plus STREAMINFO/VORBIS_COMMENT/PICTURE metadata |
...
| `GsmRaw` | Raw GSM 06.10 (.gsm) | WORM | — | — | — | — | [RPE-LTP reference][gsm] vectors | Raw GSM 06.10 frames |
...
| `Mkv` | MKV / WebM | WORM | — | — | ✅ | ✅ | — | Matroska/WebM audio-track routing, attachments/lacing as implemented; packet-preserving WebM Opus/Vorbis demux and mux |
| `Mod` | MOD (ProTracker / SoundTracker) | R | — | — | — | — | — | ProTracker/SoundTracker MOD render + samples |
| `Mp3` | MP3 (MPEG audio) | WORM | ✅ | ✅ | ✅ | ✅ | [ffmpeg][ffmpeg] on [LAME][lame] streams | MP3 — ID3v1/v2 tags + decoded channels; fresh MP3 stream construction surface |
| `Mp4` | MP4 / MOV (demuxed) | WORM | — | ✅ | — | ✅ | [ffmpeg][ffmpeg] AAC-LC track, to the exact length | MP4/MOV audio tracks routed to AAC/ALAC/MP3/AC-3/FLAC/Opus/PCM; video stays a carried track |
...
| `Nelly` | Nellymoser Asao (Flash audio stream) | R | — | — | — | — | [ffmpeg][ffmpeg] stream, RMS within 4 counts | Raw Nellymoser block stream |
| `Nsf` | NES Sound Format | R | — | — | — | — | — | NES NSF/NSFE render via 6502+2A03 plus supported expansions |
| `Ogg` | OGG (Xiph container) | WORM | ✅ | ✅ | — | — | [ffmpeg][ffmpeg] FLAC-in-Ogg stream, byte-exact | Ogg packet blobs, comments, Vorbis/Opus routes |
...
| `Opus` | Opus (Ogg) | R | ✅ | — | — | — | [libopus][opus] stream, trimmed to the exact granule | Opus-in-Ogg route; decodes at 48 kHz, trimmed to the final granule; codec limits apply |
...
| `Qoa` | Quite OK Audio (QOA) | WORM | ✅ | ✅ | — | — | [ffmpeg][ffmpeg] stream, byte-exact | Quite OK Audio container; decode verified bit-exact against libavcodec |
| `RealMedia` | RealMedia / RealAudio | WORM | — | — | ✅ | ✅ | [ffmpeg][ffmpeg] RealAudio 14.4 stream, byte-exact | RealMedia, codec-routed audio channels; mux and remux write encoded audio packets back into an .rm container |
| `Rf64` | RF64 / BWF (Broadcast Wave) | WORM | — | — | — | — | [ffmpeg][ffmpeg] probes and decodes ours | RF64/BWF, `ds64` sizes + `bext` |
...
| `Sbc` | Bluetooth SBC / mSBC | R | — | — | — | — | [ffmpeg][ffmpeg] stream, byte-exact | Raw Bluetooth SBC/mSBC |
...
| `Tta` | True Audio (TTA) | WORM | ✅ | ✅ | — | — | [ffmpeg][ffmpeg] stream, byte-exact | True Audio lossless split + assemble; decode verified bit-exact against libavcodec, 8/16/24/32-bit |
...
| `Wav` | WAV (RIFF audio) | WORM | ✅ | ✅ | ✅ | ✅ | [ffmpeg][ffmpeg] both ways; [ITU-T G.191][g191] on the G.726 route | RIFF WAV — INFO/LIST/bext metadata, multi-channel layout |
| `WavArc` | WavArc | R | — | — | — | — | — | WavArc 0CPY/1DIF paths; adaptive-LPC variants documented as limited |
| `Wave64` | Wave64 (.w64) | WORM | — | — | — | — | [ffmpeg][ffmpeg] probes and decodes ours | Sony Wave64 GUID-keyed chunks, 64-bit sizes |
| `WavPack` | WavPack (.wv) | R | ✅ | ✅ | ✅ | ✅ | [wvunpack][wavpack] verifies ours; [ffmpeg][ffmpeg] stream byte-exact | WavPack lossless/hybrid container |
...
[gsm]: https://quut.com/gsm/
[g191]: https://www.itu.int/rec/T-REC-G.191
[a52]: https://www.atsc.org/wp-content/uploads/2015/03/A52-201212-17.pdf
...
Every public and protected member of all 1277 types, generated from the built assembly and its XML documentation, is in [REFERENCE.md](https://github.com/Hawkynt/CompressionWorkbench/blob/main/Hawkynt.FileFormats.Audio/REFERENCE.md).
...
- **[vgmstream](https://github.com/vgmstream/vgmstream)** — long-tail video-game audio reference;
  ...
  container/codec combinations. For AKB specifically, [`src/meta/akb.c`](https://github.com/vgmstream/vgmstream/blob/master/src/meta/akb.c)
  pins the classic/AKB2 field layout and codec mapping while [`src/meta/sqex_streamfile.h`](https://github.com/vgmstream/vgmstream/blob/master/src/meta/sqex_streamfile.h)
  documents the sdlib `EncKey` XOR transport. vgmstream itself is decode-only, so encoder/muxer work
  requires independent format evidence rather than assuming a reverse path exists upstream.
- **[Memoria](https://github.com/Albeoris/Memoria)** — independent FFIX AKB2 writer/oracle;
  [`AKB2Header.cs`](https://github.com/Albeoris/Memoria/blob/main/Memoria.Prime/OggFile/AKB2/AKB2Header.cs)
  and [`SoundImporter.cs`](https://github.com/Albeoris/Memoria/blob/main/Assembly-CSharp/Global/Sound/Loader/SoundImporter.cs)
  pin the canonical 304-byte single-material Ogg layout and loop metadata used to cross-check the AKB2 writer.
