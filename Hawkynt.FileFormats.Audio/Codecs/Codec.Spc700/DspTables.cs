...
/// <summary>
/// Verbatim S-DSP lookup tables: the 512-entry gaussian interpolation table and the 32-entry
/// envelope rate table. Both are fixed hardware constants documented in every SNES audio
/// reference (anomie's S-DSP doc, fullsnes, the bsnes/snes9x sources); they are reproduced
/// here unchanged so envelope timing and interpolation match real hardware.
/// </summary>
internal static class DspTables {

  /// <summary>
  /// The 512-point gaussian FIR window the S-DSP uses for 4-tap sample interpolation. Indexed
  /// by the high 8 bits of the 12-bit pitch fraction (and 255-index for the mirror taps).
  /// </summary>
  public static readonly short[] Gaussian = [
  ...
  /// <summary>
  /// The 32 envelope rates as counter periods (number of 32&#160;kHz ticks between steps).
  /// Rate index 0 means "never" (the envelope holds); the rest follow the documented
  /// geometric schedule. Used for ADSR attack/decay/sustain and the linear/exponential
  /// GAIN modes.
  /// </summary>
  public static readonly int[] RatePeriod = [
