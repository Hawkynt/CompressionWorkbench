...
/// <summary>
/// Exposes a 3DS <c>.bcstm</c> (CSTM stream) as an archive of <c>FULL.bcstm</c> plus one decoded
/// mono WAV per channel (named per <see cref="ChannelLayout"/>) plus a <c>metadata.ini</c> describing
/// sample rate, channels, coding type and loop points. DSP-ADPCM (coding 2), PCM16 (1) and PCM8 (0)
/// are decoded; anything the reader cannot parse falls back gracefully to <c>FULL.bcstm</c> only.
/// </summary>
public sealed class BcstmFormatDescriptor : IFormatDescriptor, IArchiveFormatOperations,
  IArchiveInMemoryExtract, IArchiveWriteConstraints, IArchiveCreatable {

  /// <summary>
  /// Gets the id.
  /// </summary>
  public string Id => "Bcstm";
  /// <summary>
  /// Gets the display name.
  /// </summary>
  public string DisplayName => "BCSTM (3DS stream)";
  /// <summary>
  /// Gets the category.
  /// </summary>
  ...
  /// </summary>
  public string DefaultExtension => ".bcstm";
  /// <summary>
  /// Gets the extensions.
  /// </summary>
  public IReadOnlyList<string> Extensions => [".bcstm"];
  /// <summary>
  ...
  public IReadOnlyList<MagicSignature> MagicSignatures => [
    new("CSTM"u8.ToArray(), Confidence: 0.90),
  ];
  /// <summary>
  /// Gets the methods.
  /// </summary>
  ...
  /// </summary>
  public string Description => "BCSTM (3DS CSTM stream); full file + per-channel decoded WAVs + metadata.";
  ...
  // ── IArchiveCreatable: FULL passthrough OR per-channel mono WAVs → DSP-ADPCM BCSTM ──
  ...
  public void Create(Stream output, IReadOnlyList<ArchiveInputInfo> inputs, FormatCreateOptions options) {
    ...
    var full = fileList.FirstOrDefault(f =>
      Path.GetFileName(f.Name).Equals("FULL.bcstm", StringComparison.OrdinalIgnoreCase));
    if (full.Data != null) {
    ...
    if (channelBlobs.Count == 0)
      throw new InvalidOperationException("BCSTM archive create needs either FULL.bcstm or one or more per-channel WAVs.");
    ...
    output.Write(new BcstmWriter().Write(pcmChannels, first.SampleRate));
  }
  ...
  public string AcceptedInputsDescription =>
    "BCSTM archive accepts: FULL.bcstm, or LEFT/RIGHT/CENTER/… .wav (per-channel mono 16-bit)";
  ...
  private static IReadOnlyList<AudioPseudoArchive.Entry> BuildEntries(Stream stream) {
    ...
    var entries = new List<AudioPseudoArchive.Entry> {
      new("FULL.bcstm", "Container", blob),
    };

    try {
      var parsed = new BcstmReader().Read(blob);
      ...
      // Unparseable / unsupported CSTM: surface FULL only.
    }

    return entries;
  }

  private static byte[] BuildMetadata(BcstmReader.StreamInfo info) {
