...
public static class ShellRegistrar {
  ...
  /// <summary>
  /// Archive file extensions that get context menu entries.
  /// </summary>
  private static readonly string[] ArchiveExtensions = [
    ".zip", ".rar", ".7z", ".tar", ".gz", ".tgz", ".bz2", ".xz", ".zst", ".lz4",
    ".cab", ".lzh", ".lha", ".arj", ".arc", ".zoo", ".ace", ".sqx", ".cpio",
    ".ar", ".wim", ".rpm", ".deb", ".lzo", ".br", ".lzma", ".lz", ".zlib",
    ".snappy", ".sz", ".shar", ".pak",
    // Compound tar
  ...
  public static void Register(string cwbExePath, string uiExePath) {
    ...
    foreach (var ext in ArchiveExtensions) {
      using var key = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{ext}\shell\{AppName}");
    ...
    foreach (var ext in ArchiveExtensions) {
      using var key = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{ext}\shell\{AppName}.ExtractHere");
      if (key == null) continue;
    ...
    foreach (var ext in ArchiveExtensions) {
      using var key = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{ext}\shell\{AppName}.ExtractToFolder");
      if (key == null) continue;
  ...
  private static void RegisterAddToArchive(string cwbExePath, string uiExePath, string fileClass) {
    // Add to .zip
    using var zipKey = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{fileClass}\shell\{AppName}.AddToZip");
    if (zipKey != null) {
    ...
    // Add to .7z
    using var szKey = Registry.CurrentUser.CreateSubKey($@"Software\Classes\{fileClass}\shell\{AppName}.AddTo7z");
    if (szKey != null) {
